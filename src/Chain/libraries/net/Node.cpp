...
#undef DECLARE_ACCUMULATOR
            ...
            public:
                ...
                std::vector<ItemHashType> get_blockchain_synopsis(uint32_t item_type,
                    ...
                    uint64_t number_of_blocks_after_reference_point = 0) override;
                void     sync_status(uint32_t item_type, uint32_t item_count) override;
                void     connection_count_changed(uint32_t c) override;
                uint64_t get_block_number(const ItemHashType& block_id) override;
				...
				SignedBlockHeader get_blockchain_last_block() const override;
				vector<uint64_t> get_index_list() const override;
				bool on_new_connection(fc::string addr) const override;
				//void remove_status_node(fc::string addr) const override;
				...
				//vector<FullBlock> get_sync_reply_blocks(BlockIdType block_id) const override;
				//vector<FullBlock> get_sync_reply_blocks_v2(vector<uint64_t> numbers) const override;
				//void put_sync_reply_blocks(vector<FullBlock> block_list) const override;
			};
...
#define BLACKLIST_DATABASE_FILENAME      "blacklist.leveldb"
				...
				//PeerConnection* sync_peer = nullptr;
                ...
                uint64_t _total_number_of_unfetched_items; /// the number of items we still need to fetch while syncing
				...
				vector<Message> _pending_messages;
                ...
                uint64_t calculate_unsynced_block_count_from_all_peers();
				...
				//void on_sync_request_message(PeerConnection* originating_peer, const SyncRequestMessage& message);
				//void on_sync_reply_message(PeerConnection* originating_peer, const SyncReplyMessage& message);
				...
				void forecast(const Message& item_to_broadcast, int count);
				void send(const Message& item_to_broadcast, int index);
				std::string send_to_addr(const Message& item_to_broadcast, string& addr);
				std::string send_to_seed(const Message& item_to_broadcast, int index);
				std::string send_to_status(const Message& item_to_broadcast);
				int active_connection_count();
				int active_seed_connection_count();

				bool is_exist_peer(fc::string addr);
...
#define MAXIMUM_NUMBER_OF_BLOCKS_TO_PREFETCH (10 * MAXIMUM_NUMBER_OF_BLOCKS_TO_HANDLE_AT_ONE_TIME)
                    ...
                    try
                    {
                        ...
                        while (is_wanting_new_connections())
                        {
                            ...
                            for (PeerDatabase::iterator iter = _potential_peer_db.begin();
                                ...
                                ++iter)
                            {
								fc::microseconds delay_until_retry = fc::seconds((iter->number_of_failed_connection_attempts + 1) * 5); // _peer_connection_retry_timeout);
            ...
            void NodeImpl::advertise_inventory_loop()
            {
                ...
                while (!_advertise_inventory_loop_done.canceled())
                {
					...
					for (const PeerConnectionPtr& peer : _active_connections)
					{
						...
						for (const auto& message : _pending_messages) {
                    ...
                    //    {
                    //        std::map<uint64_t, std::vector<ItemHashType> > items_to_advertise_by_type;
            ...
            void NodeImpl::terminate_inactive_connections_loop()
            {
                ...
                uint32_t active_disconnect_timeout = std::max<uint32_t>(ALP_BLOCKCHAIN_BLOCK_INTERVAL_SEC * 5, 30);
                uint32_t active_send_keepalive_timeount = std::max<uint32_t>(active_disconnect_timeout / 2, 11);
                uint32_t active_ignored_request_timeount = std::max<uint32_t>(ALP_BLOCKCHAIN_BLOCK_INTERVAL_SEC * 2, 10);
            ...
            void NodeImpl::on_message(PeerConnection* originating_peer, const Message& received_message)
            {
				...
				//case CoreMessageTypeEnum::sync_request_message_type: {
				//	SyncRequestMessage message = received_message.as<SyncRequestMessage>();
				//	fc::async([this, originating_peer, message]() {
				//		on_sync_request_message(originating_peer, message);
				//	}, "on_sync_request_message");
				//	break;
				//	//on_sync_request_message(originating_peer, received_message.as<SyncRequestMessage>());
				//}
				//case CoreMessageTypeEnum::sync_reply_message_type: {
				//	SyncReplyMessage message = received_message.as<SyncReplyMessage>();
				//	fc::async([this, originating_peer, message]() {
				//		on_sync_reply_message(originating_peer, message);
				...
				default:
					...
					//if (is_syncing && received_message.msg_type != client::MessageTypeEnum::trx_message_type) break;
...
//                            uint64_t head_block_num = _delegate->get_block_number(_delegate->get_head_block_id());
...
//					}
            ...
            void NodeImpl::on_address_message(PeerConnection* originating_peer, const AddressMessage& address_message_received)
            {
                ...
                if (_handshaking_connections.find(originating_peer->shared_from_this()) != _handshaking_connections.end())
                {
                    ...
                    else
                    {
						...
						fc::string addr = fc::string(*originating_peer->get_remote_endpoint_1());
						_delegate->on_new_connection(addr);
						//if (_delegate->is_status_node_in_waiting()) {
						//	originating_peer->send_message(client::StatusRequestMessage());
						//}
					}
                }
            ...
            uint64_t NodeImpl::calculate_unsynced_block_count_from_all_peers()
            {
                VERIFY_CORRECT_THREAD();
                uint64_t max_number_of_unfetched_items = 0;
                for (const PeerConnectionPtr& peer : _active_connections)
                {
					uint64_t this_peer_number_of_unfetched_items = peer->ids_of_items_to_get.size() + peer->number_of_unfetched_item_ids;
            ...
            std::vector<ItemHashType> NodeImpl::create_blockchain_synopsis_for_peer(const PeerConnection* peer)
            {
                ...
                uint64_t reference_point_block_num = peer->last_block_number_delegate_has_seen;
                ...
                if (number_of_blocks_after_reference_point)
                {
                    ...
                    uint64_t true_high_block_num = reference_point_block_num + number_of_blocks_after_reference_point;
                ...
                const BlockchainItemIdsInventoryMessage& blockchain_item_ids_inventory_message_received)
            {
                ...
                if (originating_peer->item_ids_requested_from_peer)
                {
                        ...
                        originating_peer->number_of_unfetched_item_ids == 0) // <-- is the last check necessary?
                    {
                        ...
                        uint64_t new_number_of_unfetched_items = calculate_unsynced_block_count_from_all_peers();
            ...
            void NodeImpl::on_connection_closed(PeerConnection* originating_peer)
            {
				...
				//if (is_syncing && sync_peer == originating_peer) {
				//	for (const PeerConnectionPtr peer : _active_connections)
				//	{
				//		//if (sync_peer == peer) continue;

				//		// 이를 위해 자기 마지막블록의 아이디를 피어에 보낸다.
				//		//SignedBlockHeader last_block = _delegate->get_blockchain_last_block();
				//		SyncRequestMessage sync_msg;
				//		sync_msg.numbers = _delegate->get_index_list();
				//		//sync_msg.block_id = last_block.block_num == 0 ? BlockIdType() : last_block.id();
				//		peer->send_message(sync_msg);
				//	}

				//	sync_peer = nullptr;
				//}
				//_delegate->remove_status_node(fc::string(*originating_peer->get_remote_endpoint_1()));
...
//					}
			...
			//void NodeImpl::on_sync_request_message(PeerConnection* originating_peer, const SyncRequestMessage& message)
			//{
			//	//VERIFY_CORRECT_THREAD();
			...
			//	//
			//	//// 동기화 메시지를 받으면 자기의 마지막블록과 비교한다.
			//	////SignedBlockHeader last_block = _delegate->get_blockchain_last_block();
			//	////// 마지막블록과 같으면 이미 동기화 된것이므로 빈 메시지를 보낸다.
			//	////if (message.block_id != last_block.id()) {
			//	//	// 응답블록 리스트를 얻어온다.
			//	//vector<FullBlock> block_list = _delegate->get_sync_reply_blocks_v2(message.numbers);
			//	////vector<FullBlock> block_list = _delegate->get_sync_reply_blocks(message.block_id);

			//	//// 응답블록리스트를 요청을 보낸 피어에 전송한다.
			//	//reply.block_list = block_list;
			//	////}
			//	////else {
			//	////	ilog("same last block id received");
			//	////}

			//	//originating_peer->send_message(reply);
			//}

			//void NodeImpl::on_sync_reply_message(PeerConnection* originating_peer, const SyncReplyMessage& message)
			//{
			//	//VERIFY_CORRECT_THREAD();

			//	////ilog("sync replied: ${peer}!", ("peer", fc::string(*(originating_peer->get_remote_endpoint_1()))));

			//	//if (sync_peer != nullptr) {
			//	//	if(sync_peer != originating_peer) return;

			//	//	// 빈 응답을 받으면 동기화를 끝낸다.
			//	//	if (sync_peer == originating_peer && message.block_list.size() == 0) {
			//	//		sync_peer = nullptr;
			//	//		is_syncing = false;
			//	//		return;
			//	//	}
			...
			//	//// 동기화 응답메시지를 받으면 응답블록리스트를 처리하고 
			//	//// 다음 요청을 위한 블록아이디를 선택한다.
			//	//BlockIdType block_id = _delegate->put_sync_reply_blocks(message.block_list);

			//	//if (block_id != BlockIdType()) {
			//	//	if (sync_peer == nullptr) {
			//	//		sync_peer = originating_peer;
			//	//		is_syncing = true;
			//	//	}
			//	//	SyncRequestMessage reply;
			//	//	reply.numbers = _delegate->get_index_list();
			//	//	//reply.block_id = block_id;
			//	//	originating_peer->send_message(reply);
			//	//}
			//	//else {
			//	//	if (sync_peer == originating_peer) {
			//	//		sync_peer = nullptr;
			//	//		is_syncing = false;
			//	//		return;
			//	//	}
                ...
                const Message& message_to_process, const MessageHashType& message_hash)
            {
                ...
                //{
                    ...
                    try
                    {
						if (message_to_process.msg_type == client::MessageTypeEnum::seed_open_message_type) {
							client::SeedOpenMessage message(message_to_process.as<client::SeedOpenMessage>());
							message.peer_addr = fc::string(*originating_peer->get_remote_endpoint_1());
							_delegate->handle_message(message, false);
						}
						else if (message_to_process.msg_type == client::MessageTypeEnum::seed_close_message_type) {
							client::SeedCloseMessage message(message_to_process.as<client::SeedCloseMessage>());
							message.peer_addr = fc::string(*originating_peer->get_remote_endpoint_1());
							_delegate->handle_message(message, false);
						}
						else if (message_to_process.msg_type == client::MessageTypeEnum::sync_request_message_type) {
							client::SyncRequestMessage message(message_to_process.as<client::SyncRequestMessage>());
							message.peer_addr = fc::string(*originating_peer->get_remote_endpoint_1());
							_delegate->handle_message(message, false);
						}
						else if (message_to_process.msg_type == client::MessageTypeEnum::sync_reply_message_type) {
							client::SyncReplyMessage message(message_to_process.as<client::SyncReplyMessage>());
							message.peer_addr = fc::string(*originating_peer->get_remote_endpoint_1());
							_delegate->handle_message(message, false);
						}
						else if (message_to_process.msg_type == client::MessageTypeEnum::block_message_type) {
							client::BlockMessage message(message_to_process.as<client::BlockMessage>());
							message.peer_addr = fc::string(*originating_peer->get_remote_endpoint_1());
							_delegate->handle_message(message, false);
						}
						else if (message_to_process.msg_type == client::MessageTypeEnum::status_request_message_type) {
							client::StatusRequestMessage message(message_to_process.as<client::StatusRequestMessage>());
							message.peer_addr = fc::string(*originating_peer->get_remote_endpoint_1());
							_delegate->handle_message(message, false);
						}
						else if (message_to_process.msg_type == client::MessageTypeEnum::status_reply_message_type) {
							client::StatusReplyMessage message(message_to_process.as<client::StatusReplyMessage>());
							message.peer_addr = fc::string(*originating_peer->get_remote_endpoint_1());
							_delegate->handle_message(message, false);
						}
						else if (message_to_process.msg_type == client::MessageTypeEnum::status_start_message_type) {
							client::StatusStartMessage message(message_to_process.as<client::StatusStartMessage>());
							message.peer_addr = fc::string(*originating_peer->get_remote_endpoint_1());
							_delegate->handle_message(message, false);
						}
						else if (message_to_process.msg_type == client::MessageTypeEnum::wallet_unlock_request_message_type) {
							client::WalletUnlockRequestMessage message(message_to_process.as<client::WalletUnlockRequestMessage>());
							message.peer_addr = fc::string(*originating_peer->get_remote_endpoint_1());
							_delegate->handle_message(message, false);
						}
						else {
                ...
                const fc::ip::endpoint& remote_endpoint)
            {
                ...
                try
                {
					...
					// 접속 성공하면 실패회수를 0으로 한다.
					updated_peer_entry.number_of_failed_connection_attempts = 0;
                ...
                if (connect_failed_exception && !new_peer->performing_firewall_check())
                {
                    ...
                    PotentialPeerEntry updated_peer_entry = _potential_peer_db.lookup_or_create_entry_for_endpoint(remote_endpoint);
                    updated_peer_entry.last_connection_disposition = last_connection_failed;
                    if(updated_peer_entry.number_of_failed_connection_attempts < 60) updated_peer_entry.number_of_failed_connection_attempts++;
    ...
    //            }
				...
				//}
				//else if (item_to_broadcast.msg_type == thinkyoung::client::reply_message_type)
				//{
				//	thinkyoung::client::ReplyMessage reply_message_to_broadcast = item_to_broadcast.as<thinkyoung::client::ReplyMessage>();
				//	//hash_of_message_contents = reply_message_to_broadcast.block_id; // for debugging
				//	//dlog("broadcasting trx: ${trx}", ("trx", reply_message_to_broadcast));
				//}
				//else if (item_to_broadcast.msg_type == thinkyoung::client::missing_request_message_type)
				//{
				//	thinkyoung::client::MissingRequestMessage missing_request_message_to_broadcast = item_to_broadcast.as<thinkyoung::client::MissingRequestMessage>();
				//	//hash_of_message_contents = missing_request_message_to_broadcast.block_id; // for debugging
				//	//dlog("broadcasting trx: ${trx}", ("trx", missing_request_message_to_broadcast));
				//}
				//else if (item_to_broadcast.msg_type == thinkyoung::client::missing_reply_message_type)
				//{
				//	thinkyoung::client::MissingReplyMessage missing_reply_message_to_broadcast = item_to_broadcast.as<thinkyoung::client::MissingReplyMessage>();
				//	//hash_of_message_contents = missing_reply_message_to_broadcast.block_id; // for debugging
				//	//dlog("broadcasting trx: ${trx}", ("trx", missing_reply_message_to_broadcast));
				//}
				//else if (item_to_broadcast.msg_type == thinkyoung::client::seed_open_message_type)
				//{
				//	thinkyoung::client::SeedOpenMessage missing_reply_message_to_broadcast = item_to_broadcast.as<thinkyoung::client::SeedOpenMessage>();
				//}
				//else if (item_to_broadcast.msg_type == thinkyoung::client::seed_close_message_type)
				//{
				//	thinkyoung::client::SeedCloseMessage missing_reply_message_to_broadcast = item_to_broadcast.as<thinkyoung::client::SeedCloseMessage>();
				//}
				...
				_pending_messages.push_back(item_to_broadcast);
			...
			void NodeImpl::forecast(const Message& item_to_broadcast, int count)
			{
				VERIFY_CORRECT_THREAD();

				int index[ALP_NET_DEFAULT_DESIRED_CONNECTIONS];
				PeerConnectionPtr peers[ALP_NET_DEFAULT_DESIRED_CONNECTIONS];

				if (count > _active_connections.size()) count = _active_connections.size();

				int i;
				for (i = 0; i < _active_connections.size(); i++) {
					index[i] = i;
				}
				i = 0;
				for (PeerConnectionPtr peer : _active_connections) {
					peers[i++] = peer;
				}
				for (i = 0; i < _active_connections.size(); i++) {
					int j = rand() % _active_connections.size();
					int k = index[i];
					index[i] = index[j];
					index[j] = k;
				}
				for (i = 0; i < count; i++) {
					PeerConnectionPtr peer = peers[index[i]];
					peer->send_message(item_to_broadcast);
				}
			}

			void NodeImpl::send(const Message& item_to_broadcast, int index)
			{
				int count = _active_connections.size();
				if (count == 0) return;
			...
			std::string NodeImpl::send_to_addr(const Message& item_to_broadcast, string& addr)
			{
				std::string strEndPoint = "";
				for (const PeerConnectionPtr& peer : _active_connections)
				{
					if (peer->is_match_endpoint(addr)) {
						peer->send_message(item_to_broadcast);
						strEndPoint = addr;
						return addr;
					}
			...
			std::string NodeImpl::send_to_seed(const Message& item_to_broadcast, int index)
			{
				std::string strEndPoint = "";
				int count = active_seed_connection_count();
				if (count == 0) return strEndPoint;
				...
				for (const PeerConnectionPtr& peer : _active_connections)
				{
					if (peer->is_seed()) {
						if (index == 0) {
							peer->send_message(item_to_broadcast);							
							strEndPoint = fc::string(* (peer->get_remote_endpoint_1()));
							return strEndPoint;
						}
						index--;
					}
			...
			std::string NodeImpl::send_to_status(const Message& msg)
			{
				//for (const PeerConnectionPtr& peer : _active_connections)
				//{
				//	if (peer->is_seed()) {
			...
			int NodeImpl::active_connection_count()
			{
				return _active_connections.size();
			}

			int NodeImpl::active_seed_connection_count()
			{
				int count = 0;
				for (const PeerConnectionPtr& peer : _active_connections)
				{
					if (peer->is_seed()) count++;
				}
				return count;
			}

			bool NodeImpl::is_exist_peer(fc::string addr)
			{
				for (const PeerConnectionPtr& peer : _active_connections)
				{
					if (peer->is_match_endpoint(addr)) {
            ...
            bool NodeImpl::is_hard_fork_block(uint64_t block_number) const
            {
                return std::binary_search(_hard_fork_block_numbers.begin(), _hard_fork_block_numbers.end(), block_number);
            }
            uint32_t NodeImpl::get_next_known_hard_fork_block_number(uint64_t block_number) const
            {
...
# define INVOKE_IN_IMPL(method_name, ...) \
    return my->method_name(__VA_ARGS__)
		...
		//bool Node::is_synced() const
		//{
		//	INVOKE_IN_IMPL(is_synced);
		//}
		...
		void Node::forecast(const Message& msg, int count)
		{
			INVOKE_IN_IMPL(forecast, msg, count);
		}

		void Node::send(const Message& msg, int index)
		{
			INVOKE_IN_IMPL(send, msg, index);
		}

		std::string Node::send_to_addr(const Message& msg, string& addr)
		{
			INVOKE_IN_IMPL(send_to_addr, msg, addr);
		}

		std::string Node::send_to_seed(const Message& msg, int index)
		{
			INVOKE_IN_IMPL(send_to_seed, msg, index);
		}

		std::string Node::send_to_status(const Message& msg)
		{
			INVOKE_IN_IMPL(send_to_status, msg);
		}

		int Node::active_connection_count()
		{
			INVOKE_IN_IMPL(active_connection_count);
		}

		int Node::active_seed_connection_count()
		{
			INVOKE_IN_IMPL(active_seed_connection_count);
		}

		bool Node::is_exist_peer(fc::string addr)
		{
			INVOKE_IN_IMPL(is_exist_peer, addr);
		}
		...
		void SimulatedNetwork::forecast(const Message& item_to_broadcast, int count)
		{
			int index[ALP_NET_DEFAULT_DESIRED_CONNECTIONS];
			node_info* nodes[ALP_NET_DEFAULT_DESIRED_CONNECTIONS];

			if (count > network_nodes.size()) count = network_nodes.size();

			int i;
			for (i = 0; i < network_nodes.size(); i++) {
				index[i] = i;
			}
			i = 0;
			for (node_info* network_node_info : network_nodes) {
				nodes[i++] = network_node_info;
			}
			for (i = 0; i < network_nodes.size(); i++) {
				int j = rand() % network_nodes.size();
				int k = index[i];
				index[i] = index[j];
				index[j] = k;
			}
			for (i = 0; i < count; i++) {
				node_info* network_node_info = nodes[index[i]];
				if (network_node_info == 0) continue;
				network_node_info->messages_to_deliver.emplace(item_to_broadcast);
				if (!network_node_info->message_sender_task_done.valid() || network_node_info->message_sender_task_done.ready())
					network_node_info->message_sender_task_done = fc::async([=]() { message_sender(network_node_info); }, "simulated_network_sender");
			}
...
#define INVOKE_AND_COLLECT_STATISTICS(returnValue,method_name, ...) \
            ...
            uint64_t StatisticsGatheringNodeDelegateWrapper::get_block_number(const ItemHashType& block_id)
            {
                INVOKE_AND_COLLECT_STATISTICS(uint64_t, get_block_number, block_id);
			...
			SignedBlockHeader StatisticsGatheringNodeDelegateWrapper::get_blockchain_last_block() const
			{
				INVOKE_AND_COLLECT_STATISTICS(SignedBlockHeader, get_blockchain_last_block);
			}

			vector<uint64_t> StatisticsGatheringNodeDelegateWrapper::get_index_list() const
			{
				INVOKE_AND_COLLECT_STATISTICS(vector<uint64_t>, get_index_list);
			}

			bool StatisticsGatheringNodeDelegateWrapper::on_new_connection(fc::string addr) const
			{
				INVOKE_AND_COLLECT_STATISTICS(bool, on_new_connection, addr);
			}
			...
			//vector<FullBlock> StatisticsGatheringNodeDelegateWrapper::get_sync_reply_blocks(BlockIdType block_id) const
			//{
			//	if (_node_delegate != nullptr) {
			//		return _node_delegate->get_sync_reply_blocks(block_id);
			//	}
			//	return vector<FullBlock>();
			//}

			//vector<FullBlock> StatisticsGatheringNodeDelegateWrapper::get_sync_reply_blocks_v2(vector<uint64_t> numbers) const
			//{
			//	if (_node_delegate != nullptr) {
			//		return _node_delegate->get_sync_reply_blocks_v2(numbers);
			//	}
			//	return vector<FullBlock>();
			//}

			//void StatisticsGatheringNodeDelegateWrapper::put_sync_reply_blocks(vector<FullBlock> block_list) const
			//{
			//	if (_node_delegate != nullptr) {
			//		return _node_delegate->put_sync_reply_blocks(block_list);
			//	}
			//	//return BlockIdType();
			//}
