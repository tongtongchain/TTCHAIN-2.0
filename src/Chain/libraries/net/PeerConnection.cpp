...
namespace thinkyoung {
    namespace net
    {
        ...
        void PeerConnection::send_queueable_message(std::unique_ptr<QueuedMessage>&& message_to_send)
        {
            ...
            if (_total_queued_messages_size > ALP_NET_MAXIMUM_QUEUED_MESSAGES_IN_BYTES)
            {
                elog("send queue exceeded maximum size of ${max} bytes (current size ${current} bytes)",
                    ("max", ALP_NET_MAXIMUM_QUEUED_MESSAGES_IN_BYTES)("current", _total_queued_messages_size));
                try
                {
                    close_connection();
                }
		...
		bool PeerConnection::is_match_endpoint(string endpoint)
		{
			if (endpoint == fc::string(*_remote_endpoint)) 
				return true;
			return false;
		}

		bool PeerConnection::is_seed()
		{
			for (int i = 0; i < thinkyoung::client::SeedNodes.size(); i++) {
				if (thinkyoung::client::SeedNodes[i] == fc::string(*_remote_endpoint)) return true;
			}
			return false;
		}

		bool PeerConnection::is_status()
		{
			return false;
		}
		...
		fc::optional<fc::ip::endpoint> PeerConnection::get_remote_endpoint_1()
		{
        ...
        {
            VERIFY_CORRECT_THREAD();
            fc::time_point_sec oldest_inventory_to_keep(fc::time_point::now() - fc::minutes(ALP_NET_MAX_INVENTORY_SIZE_IN_MINUTES));
        ...
        {
            VERIFY_CORRECT_THREAD();
            return inventory_peer_advertised_to_us.size() > ALP_NET_MAX_INVENTORY_SIZE_IN_MINUTES * ALP_BLOCKCHAIN_MAX_TRX_PER_SECOND * 60;
        }
        ...
        bool PeerConnection::is_inventory_advertised_to_us_list_full() const
        {
            ...
            // plus the maximum number of blocks that would be generated in ALP_NET_MAX_INVENTORY_SIZE_IN_MINUTES (plus one,
            // to give us some wiggle room)
            return inventory_peer_advertised_to_us.size() >
                ALP_NET_MAX_INVENTORY_SIZE_IN_MINUTES * ALP_BLOCKCHAIN_MAX_TRX_PER_SECOND * 60 +
                (ALP_NET_MAX_INVENTORY_SIZE_IN_MINUTES + 1) * 60 / ALP_BLOCKCHAIN_BLOCK_INTERVAL_SEC;
        }
