    ...
    namespace lua {
        namespace api {
            ...
            */
            ...
            uint64_t GluaChainApi::get_header_block_num(lua_State *L)
            {
              thinkyoung::lua::lib::increment_lvm_instructions_executed_count(L, CHAIN_GLUA_API_EACH_INSTRUCTIONS_COUNT - 1);
                try{
                    thinkyoung::blockchain::TransactionEvaluationState* eval_state_ptr =
            ...
            uint64_t GluaChainApi::wait_for_future_random(lua_State *L, uint64_t next)
            {
              thinkyoung::lua::lib::increment_lvm_instructions_executed_count(L, CHAIN_GLUA_API_EACH_INSTRUCTIONS_COUNT - 1);
                try{
                    thinkyoung::blockchain::TransactionEvaluationState* eval_state_ptr =
                    ...
                    uint64_t target = eval_state_ptr->_current_state->get_head_block_num() + next;
                    if (target < next)
                        return 0;
                    return target;
                }
