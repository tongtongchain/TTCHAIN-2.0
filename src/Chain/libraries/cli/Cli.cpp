...
namespace thinkyoung {
    namespace cli {
        ...
        FC_DECLARE_EXCEPTION(cli_exception, 11000, "CLI Error")
            FC_DECLARE_DERIVED_EXCEPTION(abort_cli_command, thinkyoung::cli::cli_exception, 11001, "command aborted by user");
        FC_DECLARE_DERIVED_EXCEPTION(exit_cli_command, thinkyoung::cli::cli_exception, 11002, "exit CLI client requested by user");
            ...
            )
        {
            if (input_stream == nullptr)
                FC_CAPTURE_AND_THROW(thinkyoung::cli::exit_cli_command); //_input_stream != nullptr );
        ...
        namespace detail
        {
            ...
            public:
                ...
                CliImpl(thinkyoung::client::Client* client, std::istream* command_script, std::ostream* output_stream);
				...
				bool sandbox_set_cmd_limit()
				{
					cmd_limit = { "sandbox_open", "sandbox_close", "sandbox_register_contract",
						...
						"status_nodes_info", "status_node_unlocked_wallets_list"
					};
					return true;
				}
                    ...
                    fc::istream_ptr argument_stream)
                {
                    ...
                    catch (const thinkyoung::cli::abort_cli_command&)
                    {
                        throw;
                    }
                    catch (const fc::exception& e)
                    ...
                    if (command_is_valid)
                    {
                        ...
                        catch (const thinkyoung::cli::exit_cli_command&)
                        {
                            throw;
                        }
                        catch (const fc::exception& e)
                ...
                string get_line(const string& prompt = CLI_PROMPT_SUFFIX, bool no_echo = false)
                {
                    ...
                    return thinkyoung::cli::get_line(
                        _input_stream,
                        _out,
                        prompt,
                        no_echo,
                ...
                fc::variants parse_interactive_command(fc::buffered_istream& argument_stream, const string& command)
                {
                    try
                    {
                        const thinkyoung::api::MethodData& method_data = _rpc_server->get_method_data(command);
                    ...
                    const thinkyoung::api::MethodData& method_data)
                {
                    try {
                        ...
                        for (unsigned i = 0; i < method_data.parameters.size(); ++i)
                        {
                            ...
                            {
                                if (method_data.parameters[i].classification != thinkyoung::api::required_positional)
                                {
                                    return arguments;
                                }
                                else //if missing required argument, prompt for that argument
                                {
                                    const thinkyoung::api::ParameterData& this_parameter = method_data.parameters[i];
                            ...
                            if (method_data.parameters[i].classification == thinkyoung::api::optional_named)
                                break;
                        }
                        return arguments;
                ...
                {
                    try {
                        const thinkyoung::api::ParameterData& this_parameter = method_data.parameters[parameter_index];
                        if (this_parameter.type == "asset")
                        {
                            ...
                            return fc::variant(thinkyoung::blockchain::Asset(amount_as_int));
                        }
                        else if (this_parameter.type == "address")
                        {
                            ...
                            if (!thinkyoung::blockchain::Address::is_valid(address_string))
                            {
                            ...
                            return fc::variant(thinkyoung::blockchain::Address(address_string));
                        }
                ...
                fc::variant execute_interactive_command(const string& command, const fc::variants& arguments)
                {
                    ...
                    {
                        _quit = true;
                        thinkyoung::client::g_client_quit = true;
                        FC_THROW_EXCEPTION(thinkyoung::cli::exit_cli_command, "quit command issued");
                    }
						...
						if (_client->get_chain()->get_is_in_sandbox())
						{
							...
							if (pos == cmd_limit.end())
							{
								*_out << "In sandbox you can not execute this command! \n";
								...
								FC_THROW_EXCEPTION(thinkyoung::cli::abort_cli_command, "");
							}
                ...
                string prompt_for_input(const string& prompt, bool no_echo = false, bool verify = false)
                {
                    try {
                        ...
                        while (true)
                        {
                            ...
                            if (input.empty())
                                FC_THROW_EXCEPTION(thinkyoung::cli::abort_cli_command, "input aborted");
                    ...
                    const fc::variant& result)
                {
                    ...
                    {
                        string result_type;
                        const thinkyoung::api::MethodData& method_data = _rpc_server->get_method_data(method_name);
                        result_type = method_data.return_type;

                        if (result_type == "asset")
                        {
                            *_out << (string)result.as<thinkyoung::blockchain::Asset>() << "\n";
                        }
                        else if (result_type == "address")
                        {
                            *_out << (string)result.as<thinkyoung::blockchain::Address>() << "\n";
                        }
...
#ifdef HAVE_READLINE
                typedef std::map<string, thinkyoung::api::MethodData> MethodDataMapType;
            ...
            CliImpl::CliImpl(thinkyoung::client::Client* client, std::istream* command_script, std::ostream* output_stream)
                :_client(client)
      ...
      //rl_bind_keyseq("\\C-c", &control_c_handler);
            ...
            void CliImpl::initialize_method_data_if_necessary()
            {
                if (!_method_data_is_initialized)
                {
                    ...
                    vector<thinkyoung::api::MethodData> method_data_list = _rpc_server->get_all_method_data();
            ...
            char** CliImpl::json_completion(const char* text, int start, int end)
            {
                ...
                else
                {
                    ...
                    if (!trimmed_command_to_parse.empty())
                    {
                        ...
                        try
                        {
                            const thinkyoung::api::MethodData& method_data = cli_impl_instance->_rpc_server->get_method_data(trimmed_command_to_parse);
                            if (method_data.name == "help")
                            {
                                return rl_completion_matches(text, &json_command_completion_generator_function);
                            }
                        }
                        catch (const thinkyoung::rpc::unknown_method&)
                        {
                            // do nothing
                        }
                    }
        ...
        Cli::Cli(thinkyoung::client::Client* client, std::istream* command_script, std::ostream* output_stream)
        ...
        void Cli::display_message(const std::string& message)
        {
			if (my) {
				if (!my->_input_stream || !my->_out || my->_daemon_mode) return;
				(*my->_out) << "\n" << message << "\n" << my->get_prompt();
			}
        }
