...
namespace thinkyoung {
    namespace cli {
        ...
        string pretty_delegate_list(const vector<AccountEntry>& delegate_entrys, ClientRawPtr client)
        {
			...
			const auto next_slot_number = client->get_chain()->get_head_block().block_num + 1;
			...
			const auto next_slot_signee = client->get_chain()->get_slot_signee(next_slot_number, active_delegates);
               ...
               //vector<PrettyLedgerEntry>::const_iterator _citer = transaction.ledger_entries.begin();
                ...
                if (account_specified)
                {
                    ...
                    if (!is_pending)
                    {
						...
						fc::signed_int tempAssetID = _citer->amount.asset_id;
						/*if (transaction.trx_type == thinkyoung::blockchain::TransactionType::transfer_multi_asset){
							Asset tempAsset;
							for (auto& tempEntry : transaction.ledger_entries){
								if (tempEntry.amount.asset_id != 0){
									tempAssetID = tempEntry.amount.asset_id;
									tempAsset = tempEntry.running_balances.at(name).at(tempAssetID);
								}
							}
							out << client->get_chain()->to_pretty_asset(tempAsset) << "|";
						}
						else{
							out << client->get_chain()->to_pretty_asset(_citer->running_balances.at(name).at(tempAssetID)) << "|";
						}
*/
						out << client->get_chain()->to_pretty_asset(_citer->running_balances.at(name).at(tempAssetID)) << "|";
            ...
            for (const auto& transaction : transactions)
            {
                ...
                for (const auto& entry : transaction.ledger_entries)
                {
                    ...
                    if (count == 1)
                    {
                        ...
                        if (!is_pending)
                        {
							BlockNumberType blockNum(transaction.block_num);
							sprintf(buff, "%d:%d", blockNum.node_index(), blockNum.produced());
                            //out << transaction.block_num;
							out << buff;
                        }
                        else if (transaction.error.valid())
        ...
        string pretty_asset_list(const vector<AssetEntry>& asset_entrys, ClientRawPtr client)
        {
            ...
            for (const auto& asset_entry : asset_entrys)
            {
                ...
                out << std::setw(17) << asset_entry.symbol;
