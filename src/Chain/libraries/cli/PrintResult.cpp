...
#define blank4 "    "
			...
			command_to_function["blockchain_get_fullblock"] = &f_blockchain_get_fullblock;
			command_to_function["blockchain_get_fullblock_list"] = &f_blockchain_get_fullblock_list;
...
#endif			
			string strTrx = fc::json::to_pretty_string(trx_pair);
            //out << fc::json::to_pretty_string(trx_pair);

			BlockNumberType blockNumber(trx_pair.second.chain_location.block_num);
			BlockNumberStruct nuberStructure;
			nuberStructure.nodeIndex = blockNumber.node_index();
			nuberStructure.blockIndex = blockNumber.produced();

			out << "[\n" << strTrx << ",\n" << fc::json::to_pretty_string(nuberStructure) << "\n]";
        }
		...
		void PrintResult::f_blockchain_get_fullblock(std::ostream& out, const fc::variants& arguments, const fc::variant& result, ClientRawPtr client)
		{
			auto block = result.as<fc::mutable_variant_object>();
			out << fc::json::to_pretty_string(block) << "\n";
		}

		void PrintResult::f_blockchain_get_fullblock_list(std::ostream& out, const fc::variants& arguments, const fc::variant& result, ClientRawPtr client)
		{
			auto block = result.as<fc::mutable_variant_object>();
			out << fc::json::to_pretty_string(block) << "\n";
		}
