...
namespace thinkyoung {
    namespace blockchain {
		...
		//add zxj
		void Transaction::update_transfer_config(const std::string& delegate_account_name,
			const std::string& min_transaction_fee,
			const std::string& max_transaction_fee,
			const std::string& transaction_fee_ratio)
		{
			UpdateTransferConfigOperation op(delegate_account_name, std::stoi(min_transaction_fee), std::stoi(max_transaction_fee), std::stod(transaction_fee_ratio));
			operations.emplace_back(std::move(op));
		}
            ...
            uint64_t precision)
        {
            ...
            FC_ASSERT(max_share_supply <= ALP_BLOCKCHAIN_MAX_SHARES, "Max_share_supply must not be bigger than ALP_BLOCKCHAIN_MAX_SHARES!");
