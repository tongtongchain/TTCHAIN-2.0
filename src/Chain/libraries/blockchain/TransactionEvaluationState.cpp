...
namespace thinkyoung {
    namespace blockchain {
     ...
     //               }
					...
					////트랜잭션의 초과시간이  (2일 + 블록생성 간격)보다 더 크게 설정된 경우
     ...
     //                   FC_CAPTURE_AND_THROW(invalid_transaction_expiration, (trx_arg)(_current_state->now()));
					...
					//콘트랙트가 아닌  기본 트랜잭션만 포함한경우  최대크기를 초과하는지 검사.
					...
					//콘트랙트 트랜잭션이거나  완성되지 못한 트랜잭션인 경우  수수료 계산을 무시한다. 
                    for (const auto& op : trx.operations)
                    {
                        if (
					...
					//싸인 개수를 검사한다.
					int num_of_signature = (int)trx_arg.signatures.size();
        ...
        bool TransactionEvaluationState::is_contract_trxs_same(const SignedTransaction& l_trx, const SignedTransaction& r_trx)const
        {
            try {
                ...
                int op_size = (int)l_trx.operations.size();
