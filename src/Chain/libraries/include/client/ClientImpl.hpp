        ...
        namespace detail {
            ...
            public:
                ...
                void start()
                {
                    ...
                    {
						if (is_status)
							reschedule_status_loop();
						else
							reschedule_sync_loop();
                ...
                */
				...
				void delegate_loop();
				void broadcast_new_block(FullBlock block);
				void do_migration();

				void reschedule_sync_loop();
				void start_sync_loop();
				void cancel_sync_loop();
				void sync_loop();

				void reschedule_status_loop();
				void start_status_loop();
				void cancel_status_loop();
				void status_loop();

				void set_target_connections(uint32_t target);
				bool is_master_status_node() const;
				bool is_master_status_node(int index) const;
				void network_send_transaction(const thinkyoung::blockchain::SignedTransaction& transaction_to_broadcast, string strSeedAddr);
				...
				bool on_new_block(BlockMessage message);
				bool can_push_block(FullBlock block);

				//bool on_reply_message(const BlockIdType& block_id);
                ...
                virtual uint64_t get_block_number(const thinkyoung::net::ItemHashType& block_id) override;
				...
				virtual SignedBlockHeader get_blockchain_last_block() const override;
				virtual vector<uint64_t> get_index_list() const override;
				virtual bool on_new_connection(fc::string addr) const override;
				//virtual void remove_status_node(fc::string addr) const override;
				...
				//vector<FullBlock> get_sync_reply_blocks(BlockIdType block_id) const;
				vector<FullBlock> get_sync_reply_blocks_v2(vector<uint64_t> numbers) const;
				bool can_get_sync_blocks(vector<uint64_t> numbers) const;
				vector<FullBlock> get_new_block_list(vector<FullBlock> block_list) const;
				//void put_sync_reply_blocks(vector<FullBlock> block_list) const;

				void push_active_seed(fc::string addr) ;
				void remove_active_seed(fc::string addr) ;
				vector<fc::string> get_active_seed_list() const;
				bool is_active_seed(fc::string addr);

				void add_status_node(int index, fc::string addr, fc::time_point active_time);
				void remove_status_node_real(fc::string addr);
				void add_wallet_unlock_request(const WalletUnlockRequestMessage& msg);
				void remove_wallet_status(uint64_t wallet_id);

				void on_sync_request_message(const SyncRequestMessage& message);
				void on_sync_reply_message(const SyncReplyMessage& message);

                /// @}

				bool is_syncing = false;
				bool in_process_sync_reply = false;
				...
				int                                                     _empty_transactions_counter = 0;
				int                                                     _seed_active_counter = 0;

				mutable vector<fc::string>								_active_seed_list;
                ...
                uint64_t                                                _last_sync_status_head_block;
				...
				fc::future<void>                                        _sync_loop_complete;
				fc::future<void>                                        _status_loop_complete;
				...
				void wallet_unlock_later();
				void broadcast_to_status_nodes(net::Message msg);
				int get_active_status_node_count() const;
				...
				uint64_t string_to_block_number(const string& block);

				bool is_seed = false;
				
				bool is_status = false;
				bool status_waiting = true;
				int status_waiting_counter = 0;
				int status_index = 0;
				fc::time_point status_active_time;
				mutable vector<StatusNodeInfo> status_node_list;
				mutable vector<WalletStatusInfo> wallet_status_list;
				vector<WalletUnlockRequestMessage> wallet_request_queue;

				fc::mutex broadcast_trx_mutex;

				int sync_counter = SYNC_INTERVAL - 1;
