...
#define MIN_CHILDREN_FOR_COMPLETE_BLOCK 2
        ...
        class ChainDatabase : public ChainInterface, public std::enable_shared_from_this < ChainDatabase > {
          public:
            ...
            */
			...
			Set blobk parents
			*/
			void set_block_parents(FullBlock& block, bool min = true);
            ...
            */
			...
			void store_transaction_sending_state(TransactionIdType& tid, string& seedAddr);
            /**
            ...
            */
			...
			vector<std::pair<TransactionEvaluationStatePtr, string>>   get_pending_transactions_for_send();
			optional<TransactionSendingState> get_transaction_sending_state(TransactionIdType trx_id);
            ...
            * @return FullBlock
            */
			FullBlock					generate_block(const time_point_sec block_timestamp, const DelegateConfig& config = DelegateConfig());

			void						remove_pending_transaction(TransactionIdType id);

			vector<FullBlock>			generate_empty_blocks(const time_point_sec block_timestamp, const DelegateConfig& config = DelegateConfig());
            ...
            * @return AccountEntry
            */
            AccountEntry              get_block_signee(uint64_t block_num)const;
            ...
            * @return AccountEntry
            */
			AccountEntry              get_slot_signee(uint64_t block_num,
				const std::vector<AccountIdType>& ordered_delegates)const;

			AccountEntry              get_slot_signee1(const time_point_sec timestamp,
				const std::vector<AccountIdType>& ordered_delegates)const;
            ...
            */
            ...
            vector<AlpTrxidBalance> fetch_alp_full_entry(const uint64_t& block_num, const uint64_t& last_scan_block_num);
            ...
            * @return uint32_t
            */
            uint64_t                    find_block_num(fc::time_point_sec &time)const;
            ...
            * @return uint32_t
            */
			uint64_t                    get_block_num(const BlockIdType&)const;
			uint64_t                    get_block_rank(const BlockIdType&)const;
            ...
            * @return SignedBlockHeader
            */
            SignedBlockHeader         get_block_header(uint64_t block_num)const;
            ...
            * @return DigestBlock
            */
            DigestBlock                get_block_digest(uint64_t block_num)const;
            ...
            * @return FullBlock
            */
            FullBlock                  get_block(uint64_t block_num)const;

			optional<FullBlock> get_block_optional(const BlockIdType& block_id)const;
			optional<FullBlock> get_block_optional_in_full_db(const BlockIdType& block_id) const;
			//optional<FullBlock> get_block_optional_in_cache(const BlockIdType& block_id) const;
			//void remove_in_cache(const BlockIdType& block_id) const;
            ...
            */
			...
			SignedBlockHeader					get_old_head_block()const;
			SignedBlockHeader					get_my_head_block()const;
			void								update_head_block(const SignedBlockHeader&) const;
			void								update_old_head_block(const SignedBlockHeader&) const;
			void								update_my_head_block(const SignedBlockHeader&) const;
			vector<SignedBlockHeader>			get_parent_blocks()const;
			FullBlock							migrate_old_block(uint32_t, PublicKeyType& signee);

			uint32_t					get_old_total_blocks()const;
            ...
            * @return uint32_t
            */
			virtual uint64_t            get_head_block_num()const override;
			virtual uint64_t            get_block_count(int index = -1);
			virtual vector<uint64_t>	get_index_list();
			virtual uint64_t            get_my_head_block_num()const;
			virtual uint16_t				get_node_index()const;
            ...
            */
            vector<EventOperation>    get_events(uint64_t block_index, const thinkyoung::blockchain::TransactionIdType& trx_id);
            ...
            * @return BlockIdType
            */
            BlockIdType               get_block_id(uint64_t block_num)const;
            ...
            */
			...
			oFullBlockEntry           get_fullblock_entry(const BlockIdType& block_id)const;
            ...
            * @return oBlockEntry
            */
			oBlockEntry               get_block_entry(uint64_t block_num)const;
			oFullBlockEntry           get_fullblock_entry(uint64_t block_num)const;
            ...
            */
            std::vector<SlotEntry> get_delegate_slot_entrys(const AccountIdType delegate_id, uint64_t limit)const;
            ...
            */
            std::map<uint64_t, std::vector<ForkEntry> > get_forks_list()const;
            ...
            * @return string
            */
            std::string export_fork_graph(uint64_t start_block = 1, uint64_t end_block = -1, const fc::path& filename = "")const;            


			/*
				this function process check if block is approval from more than two child.
				if approved, store to the block db and remove from pending list.				
			*/
			//bool check_new_block_parents(const FullBlock& block);
			//bool process_reply_message(const BlockIdType& block, int peer_count);
			//vector<BlockIdType> add_block_to_cache(const BlockIdType& block_id);
			//vector<BlockIdType> get_missing_blocks();
			//bool is_exist_in_missing_request(const BlockIdType& block_id);
			//void remove_in_missing_request(const BlockIdType& block_id);
			//bool link_child_to_parent(const BlockIdType& parent_id, const BlockIdType& child_id);
			//int missing_count_in_request();
			//optional<FullBlock> find_parent_block(const BlockIdType& block_id);
			//vector<BlockIdType> get_missing_parents_by_id(const BlockIdType& block_id);
			//vector<BlockIdType> process_missing_reply_message(FullBlock& block);
			optional<FullBlock> find_block_by_id(const BlockIdType& block_id, const BlockIdType& child_id);
			//void push_in_cache(const BlockIdType& block_id, const FullBlock& block);
			void update_last_index(uint64_t block_num);
			uint64_t get_produced(uint32_t nodeindex); 
			//vector<BlockIdType> arrange_cache_blocks();

			//vector<BlockIdType> missing_blocks_in_request;
            ...
            */
            //BlockForkData
			bool push_block(const FullBlock& block_data);
            ...
            */
            vector<AccountIdType>            get_delegates_by_vote(uint64_t first = 0, uint64_t count = uint64_t(-1))const;
            ...
            * @return void
            */
            void    set_forkdb_num(uint64_t forkdb_num);
