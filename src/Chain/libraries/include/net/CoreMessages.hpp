...
namespace thinkyoung {
    namespace net {
        ...
        struct ItemId
        {
            ...
            ItemId() {}
            ItemId(uint64_t type, const ItemHashType& hash) :
                item_type(type),
                item_hash(hash)
            {}
        ...
        enum CoreMessageTypeEnum
        {
            ...
            core_message_type_first = 25000,
            item_ids_inventory_message_type = 25001,
            blockchain_item_ids_inventory_message_type = 25002,
            fetch_blockchain_item_ids_message_type = 25003,
            fetch_items_message_type = 25004,
            item_not_available_message_type = 25005,
            hello_message_type = 25006,
            connection_accepted_message_type = 25007,
            connection_rejected_message_type = 25008,
            address_request_message_type = 25009,
            address_message_type = 25010,
            closing_connection_message_type = 25011,
            current_time_request_message_type = 25012,
            current_time_reply_message_type = 25013,
            check_firewall_message_type = 25014,
            check_firewall_reply_message_type = 25015,
            get_current_connections_request_message_type = 25016,
            get_current_connections_reply_message_type = 25017,
            core_message_type_last = 25099,

			sync_request_message_type = 25201,
			sync_reply_message_type = 25202
        };
        ...
        struct ItemIdsInventoryMessage
        {
            ...
            ItemIdsInventoryMessage(uint64_t item_type, const std::vector<ItemHashType>& item_hashes_available) :
                item_type(item_type),
        ...
        struct BlockchainItemIdsInventoryMessage
        {
            ...
            uint64_t total_remaining_item_count;
            uint32_t item_type;
            std::vector<ItemHashType> item_hashes_available;
...
FC_REFLECT(thinkyoung::net::ItemId, 
	(item_type)
    (item_hash))

FC_REFLECT(thinkyoung::net::ItemIdsInventoryMessage, 
	(item_type)
    (item_hashes_available))

FC_REFLECT(thinkyoung::net::BlockchainItemIdsInventoryMessage, 
	(total_remaining_item_count)
    (item_type)
    (item_hashes_available))

FC_REFLECT(thinkyoung::net::FetchBlockchainItemIdsMessage, 
	(item_type)
    (blockchain_synopsis))

FC_REFLECT(thinkyoung::net::FetchItemsMessage, 
	(item_type)
    (items_to_fetch))

FC_REFLECT(thinkyoung::net::ItemNotAvailableMessage, (requested_item))

FC_REFLECT(thinkyoung::net::HelloMessage, 
	(user_agent)
...
FC_REFLECT_ENUM(thinkyoung::net::RejectionReasonCode, 
	(unspecified)
    (different_chain)
    (already_connected)
    (connected_to_self)
...
FC_REFLECT(thinkyoung::net::ConnectionRejectedMessage, 
	(user_agent)
    (core_protocol_version)
    (remote_endpoint)
    (reason_code)
...
FC_REFLECT(thinkyoung::net::AddressInfo, 
	(remote_endpoint)
    (last_seen_time)
    (latency)
    (node_id)
...
FC_REFLECT(thinkyoung::net::ClosingConnectionMessage, 
	(reason_for_closing)
    (closing_due_to_error)
    (error))

FC_REFLECT_ENUM(thinkyoung::net::PeerConnectionDirection, 
	(unknown)
    (inbound)
    (outbound))

FC_REFLECT_ENUM(thinkyoung::net::FirewalledState, 
	(unknown)
    (firewalled)
    (not_firewalled))
...
FC_REFLECT(thinkyoung::net::CurrentTimeReplyMessage, 
	(request_sent_time)
    (request_received_time)
    (reply_transmitted_time))
    
FC_REFLECT_ENUM(thinkyoung::net::FirewallCheckResult, 
	(unable_to_check)
    (unable_to_connect)
    (connection_successful))
...
FC_REFLECT(thinkyoung::net::CurrentConnectionData, 
	(connection_duration)
    (remote_endpoint)
    (node_id)
    (clock_offset)
...
FC_REFLECT(thinkyoung::net::GetCurrentConnectionsReplyMessage, 
