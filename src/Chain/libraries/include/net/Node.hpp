...
namespace thinkyoung {
    namespace net {
        ...
        public:
             ...
             */
            virtual std::vector<ItemHashType> get_blockchain_synopsis(uint32_t item_type, const thinkyoung::net::ItemHashType& reference_point = thinkyoung::net::ItemHashType(), uint64_t number_of_blocks_after_reference_point = 0) = 0;
            ...
            virtual uint64_t get_block_number(const ItemHashType& block_id) = 0;
			...
			virtual SignedBlockHeader get_blockchain_last_block() const = 0;
			virtual vector<uint64_t> get_index_list() const = 0;
			virtual bool on_new_connection(fc::string addr) const = 0;
			//virtual void remove_status_node(fc::string addr) const = 0;
			//virtual vector<FullBlock> get_sync_reply_blocks(BlockIdType block_id) const = 0;
			//virtual vector<FullBlock> get_sync_reply_blocks_v2(vector<uint64_t> numbers) const = 0;
			//virtual void put_sync_reply_blocks(vector<FullBlock> block_list) const = 0;
        ...
        public:
            ...
            */
			...
			virtual void  forecast(const Message& item_to_broadcast, int count);
			virtual void send(const Message& item_to_broadcast, int index);
			virtual std::string send_to_seed(const Message& item_to_broadcast, int index);
			virtual std::string send_to_status(const Message& item_to_broadcast);
			virtual std::string send_to_addr(const Message& msg, string& addr);
			virtual int active_connection_count();
			virtual int active_seed_connection_count();
        ...
        public:
			...
			void      forecast(const Message& item_to_broadcast, int count) override;
