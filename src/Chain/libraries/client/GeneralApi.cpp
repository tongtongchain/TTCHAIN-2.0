    ...
    namespace client {
        namespace detail {
            ...
            variant_object ClientImpl::get_info()const
            {
                ...
                /* Blockchain */
				SignedBlockHeader head_block = _chain_db->get_head_block();
				//BlockNumberType block_num = (BlockNumberType)head_block.block_num;

    //            const auto head_block_num = _chain_db->get_my_head_block_num();
				//const auto node_index = _chain_db->get_node_index();

				info["blockchain_head_block_nodeindex"] = head_block.nodeindex();
				info["blockchain_head_block_number"] = head_block.produced();
                info["blockchain_head_block_age"] = variant();
                info["blockchain_head_block_timestamp"] = variant();
                info["blockchain_head_block_id"] = variant();

				if (head_block.produced() > 0)
                {
					time_point_sec head_block_timestamp;
					head_block_timestamp = head_block.timestamp; //_chain_db->now();
					...
					info["blockchain_head_block_id"] = head_block.id();
                }
				...
				const auto blocks_left = ALP_BLOCKCHAIN_NUM_DELEGATES - (_chain_db->get_my_head_block_num() % ALP_BLOCKCHAIN_NUM_DELEGATES);
				...
				info["blockchain_is_synced"] = blockchain_is_synced();
                ...
                if (is_open)
                {
                    ...
                    if (unlocked_until.valid())
                    {
                        ...
                        //const auto last_scanned_block_num = _wallet->get_last_scanned_block_numbers();
