    ...
    namespace client {
        namespace detail {
            ...
            void detail::ClientImpl::wallet_open(const string& wallet_name) {
				...
				if (!is_status && _wallet->is_open()) {
					WalletCloseMessage message;
					message.wallet_id = _wallet->get_wallet_db().get_id();
					broadcast_to_status_nodes(message);
				}
            ...
            void detail::ClientImpl::wallet_close() {
                ...
                if (_wallet) {
					...
					if (is_seed) _p2p_node->broadcast(SeedCloseMessage());
                }
            }
            ...
            */
            ...
            void detail::ClientImpl::wallet_unlock(uint32_t timeout, const string& password) {
				...
				//if(!is_seed && !is_status && get_active_status_node_count() == 0)
				//	FC_THROW_EXCEPTION(no_status_node_connected, "you cannot unlock wallet until connected to status node.");
				...
				//else {
				//	_wallet->unlock_waiting(password, timeout);

				//	WalletUnlockRequestMessage message;
				//	message.wallet_id = _wallet->get_wallet_db().get_id();
				//	message.wallet_name = _wallet->get_wallet_name(); // _wallet->get_wallet_db().get_name();
				//	message.timeout = timeout;
				//	broadcast_to_status_nodes(message);

				//	//printf("\nPlease wait until to be allowed from status nodes.\n");
				//	//FC_THROW_EXCEPTION(unlock_waiting, "waiting unlock allow from status node.");
				//}
            }

			void detail::ClientImpl::wallet_unlock_later() {
            ...
            WalletTransactionEntry detail::ClientImpl::wallet_publish_slate(const string& publishing_account_name,
                ...
                _wallet->cache_transaction(entry, is_seed);
                network_broadcast_transaction(entry.trx);
                return entry;
            }
            ...
            WalletTransactionEntry detail::ClientImpl::wallet_asset_create(
				...
				/*auto entry = _wallet->update_asset(symbol, asset_name, description, public_data, 
					maximum_share_supply, precision, 100, 0.001, 0, 0, issuer_name, 0, , true);*/
            ...
            string detail::ClientImpl::wallet_import_private_key(const string& wif_key_to_import,
                ...
                if (wallet_rescan_blockchain)
                    _wallet->start_scan(vector<uint64_t>(), -1);
            ...
            void ClientImpl::wallet_rescan_blockchain(const uint64_t start_block_num, const uint32_t limit) {
                // set limit in  sandbox state
                if (_chain_db->get_is_in_sandbox())
                    FC_THROW_EXCEPTION(sandbox_command_forbidden, "in sandbox, this command is forbidden, you cannot call it!");
                ...
                try {
					...
					idxs.push_back(start_block_num-1);
                    _wallet->start_scan(idxs, limit);
                }
            ...
            std::string detail::ClientImpl::wallet_transfer_to_public_account_rpc(
                ...
                auto record = _wallet->transfer_asset_to_address(amount_to_transfer,
                              ...
                              strategy,
                              true);
                _wallet->cache_transaction(record, is_seed);
