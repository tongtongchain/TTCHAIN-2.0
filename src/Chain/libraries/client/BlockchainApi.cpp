...
#define STRING_TO_BLOCKNUM(block) \
	int colon = block.find(':');	\
	uint64_t nodeindex = _chain_db->get_node_index();	\
	uint64_t produced = 0;	\
	if (colon >= 0) {	\
		int nindex = std::stoi(block.substr(0, colon));	\
		if (nindex >= 0) nodeindex = nindex;	\
		produced = std::stoi(block.substr(colon + 1));	\
	} else produced = std::stoi(block);	\
	uint64_t block_num = BlockNumberType(produced, nodeindex).block_number()
...
namespace thinkyoung {
    namespace client {
        namespace detail {
            ...
            vector<AccountEntry> ClientImpl::blockchain_list_active_delegates(uint64_t first, uint64_t count)const {
                ...
                FC_ASSERT(first < ALP_BLOCKCHAIN_NUM_DELEGATES, "Only the first 11 delegates are active");
                FC_ASSERT(first + count <= ALP_BLOCKCHAIN_NUM_DELEGATES, "Only the first 11 delegates are active");
            ...
            vector<AccountEntry> ClientImpl::blockchain_list_delegates(uint64_t first, uint64_t count)const {
                // set limit in  sandbox state
                if (_chain_db->get_is_in_sandbox())
                    FC_THROW_EXCEPTION(sandbox_command_forbidden, "in sandbox, this command is forbidden, you cannot call it!");
            ...
            vector<string> ClientImpl::blockchain_list_missing_block_delegates(uint64_t block_num) {
                // set limit in  sandbox state
                if (_chain_db->get_is_in_sandbox())
                    FC_THROW_EXCEPTION(sandbox_command_forbidden, "in sandbox, this command is forbidden, you cannot call it!");
            ...
            vector<AlpTrxidBalance> ClientImpl::blockchain_get_alp_account_balance_entry(const uint64_t block_num) {
                // set limit in  sandbox state
                if (_chain_db->get_is_in_sandbox())
                    FC_THROW_EXCEPTION(sandbox_command_forbidden, "in sandbox, this command is forbidden, you cannot call it!");
                    
                vector<uint64_t> last_scan_nums = _wallet->get_last_scanned_block_numbers_for_alp();
				uint64_t last_scan_num = 0;
				for (int i = 0; i < last_scan_nums.size(); i++)
				{
					if (BlockNumberType(last_scan_nums[i]).node_index() == BlockNumberType(block_num).node_index())
					{
						last_scan_num = last_scan_nums[i];
						last_scan_nums[i] = block_num - 1;
						break;
					}
				}
				...
				_wallet->start_scan(last_scan_nums, -1);
                return results;
            }
            vector<BlockEntry> ClientImpl::blockchain_list_blocks(const uint64_t max_block_num, const uint64_t limit) {
                // set limit in  sandbox state
                if (_chain_db->get_is_in_sandbox())
                    FC_THROW_EXCEPTION(sandbox_command_forbidden, "in sandbox, this command is forbidden, you cannot call it!");
                ...
                try {
                    ...
                    const uint64_t head_block_num = _chain_db->get_head_block_num();
                    uint64_t start_block_num = head_block_num;
                    ...
                    const uint64_t count = std::min(limit, start_block_num);
                    vector<BlockEntry> entrys;
                    entrys.reserve(count);
                    
                    for (uint64_t block_num = start_block_num; block_num > 0; --block_num) {
            ...
            uint64_t detail::ClientImpl::blockchain_get_block_count(const string& nodeindex) const {
                // set limit in  sandbox state
                if (_chain_db->get_is_in_sandbox())
                    FC_THROW_EXCEPTION(sandbox_command_forbidden, "in sandbox, this command is forbidden, you cannot call it!");

				int index = -1;
				if (nodeindex != "") index = fc::variant(nodeindex).as<int>();

				//return _chain_db->get_head_block_num();
				return _chain_db->get_block_count(index);
            }
            
			uint64_t detail::ClientImpl::status_node_close_wallet(const string& index) const {
                // set limit in  sandbox state
                if (_chain_db->get_is_in_sandbox())
                    FC_THROW_EXCEPTION(sandbox_command_forbidden, "in sandbox, this command is forbidden, you cannot call it!");

				if (!is_status) FC_THROW_EXCEPTION(forcely_close_wallet, "not in status node, this command is forbidden, you cannot call it!");

				int i = fc::variant(index).as<int>();
				if (i < 0 || i >= wallet_status_list.size()) FC_THROW_EXCEPTION(index_invalid, "index is not in range of wallet list!");
				
				WalletForcelyCloseMessage msg;
				msg.wallet_id = wallet_status_list[i].id;
				_p2p_node->send_to_addr(msg, wallet_status_list[i].peer_addr);

				return 0;				
            }
            
			uint64_t detail::ClientImpl::status_node_lock_wallet(const string& index) const {
				// set limit in  sandbox state
				if (_chain_db->get_is_in_sandbox())
					FC_THROW_EXCEPTION(sandbox_command_forbidden, "in sandbox, this command is forbidden, you cannot call it!");
            ...
            thinkyoung::blockchain::ContractTrxInfo detail::ClientImpl::blockchain_get_contract_result(const std::string& result_id) const {
                ...
                try {
					...
					BlockNumberType blocknum(trx->chain_location.block_num);
                    thinkyoung::blockchain::ContractTrxInfo contract_result;
					contract_result.node_index = blocknum.node_index();
					contract_result.block_num = blocknum.produced();
			...
			oFullBlockEntry detail::ClientImpl::blockchain_get_block(const string& block)const
			{
				// set limit in  sandbox state
				if (_chain_db->get_is_in_sandbox())
					FC_THROW_EXCEPTION(sandbox_command_forbidden, "in sandbox, this command is forbidden, you cannot call it!");
				...
				try {
					oFullBlockEntry fullblock;

					try {
						ASSERT_TASK_NOT_PREEMPTED(); // make sure no cancel gets swallowed by catch(...)

						if (block.size() == string(BlockIdType()).size())
							fullblock = _chain_db->get_fullblock_entry(BlockIdType(block));

						else {
							STRING_TO_BLOCKNUM(block);
							fullblock = _chain_db->get_fullblock_entry(block_num);
						}

						//for (auto trx_id : fullblock->user_transaction_ids) {
						//	PrettyTransaction trx = blockchain_get_pretty_transaction(trx_id.str(), true);
						//	fullblock->user_transactions.push_back(trx);
						//}

						fullblock->signee = blockchain_get_block_signee(block);
					}
					catch (...) {
					}
			...
			oFullBlockEntry detail::ClientImpl::blockchain_get_fullblock(const string& block) const
			{
				// set limit in  sandbox state
				if (_chain_db->get_is_in_sandbox())
					FC_THROW_EXCEPTION(sandbox_command_forbidden, "in sandbox, this command is forbidden, you cannot call it!");
			...
			std::vector<oFullBlockEntry> detail::ClientImpl::blockchain_get_fullblock_list(const string& block, const uint32_t limit) const
			{
				// set limit in  sandbox state
				if (_chain_db->get_is_in_sandbox())
					FC_THROW_EXCEPTION(sandbox_command_forbidden, "in sandbox, this command is forbidden, you cannot call it!");
				...
				try {
					std::vector<oFullBlockEntry> fullblock_list;
					...
					try {
						...
						uint64_t blocknum = fullblock->block_num;
						uint32_t l = limit;
						...
						do {
							...
							fullblock->signee = _chain_db->get_block_signee(blocknum).name;

							fullblock_list.push_back(fullblock);
							...
							fullblock = _chain_db->get_fullblock_entry(blocknum);
						} while (true);
					}
					catch (...) {
					}

					return fullblock_list;
				}
            ...
            variant_object ClientImpl::blockchain_get_info()const {
                ...
                info["block_interval"] = ALP_BLOCKCHAIN_BLOCK_INTERVAL_SEC_V2;
            ...
            bool ClientImpl::blockchain_is_synced() const {
				...
				return is_syncing == false; // _p2p_node->is_synced();
            ...
            uint32_t ClientImpl::blockchain_get_total_seednode() const {
                if (_chain_db->get_is_in_sandbox())
                    FC_THROW_EXCEPTION(sandbox_command_forbidden, "in sandbox, this command is forbidden, you cannot call it!");
                    
				uint32_t count = get_active_seed_list().size();

				if (is_seed && _chain_db->get_node_index() < 0xffff && _wallet->is_open() && _wallet->is_unlocked()) {
					vector<WalletAccountEntry> enabled_delegates = _wallet->get_my_delegates(enabled_delegate_status);
					if (!enabled_delegates.empty()) count++;
				}

				return count;
            }
            
            uint32_t ClientImpl::blockchain_get_total_status_nodes() const {
                if (_chain_db->get_is_in_sandbox())
                    FC_THROW_EXCEPTION(sandbox_command_forbidden, "in sandbox, this command is forbidden, you cannot call it!");
                    
				uint32_t count = get_active_status_node_count(); //get_active_seed_list().size();
            ...
            uint32_t ClientImpl::status_node_get_index() const {
                if (_chain_db->get_is_in_sandbox())
                    FC_THROW_EXCEPTION(sandbox_command_forbidden, "in sandbox, this command is forbidden, you cannot call it!");
            ...
            uint32_t ClientImpl::status_node_unlocked_wallets_count() const {
                if (_chain_db->get_is_in_sandbox())
                    FC_THROW_EXCEPTION(sandbox_command_forbidden, "in sandbox, this command is forbidden, you cannot call it!");
                
				if (!is_status) FC_THROW_EXCEPTION(cant_exec_on_non_status, "This command can execute only on status nodes.");
				return wallet_status_list.size();
            }
            ...
            map<TransactionIdType, TransactionEntry> ClientImpl::blockchain_get_block_transactions(const string& block)const {
				...
				{
					STRING_TO_BLOCKNUM(block);
					transactions = _chain_db->get_transactions_for_block(_chain_db->get_block_id(block_num));
				}
            ...
            std::string ClientImpl::blockchain_export_fork_graph(uint64_t start_block, uint64_t end_block, const thinkyoung::blockchain::FilePath& filename)const {
                // set limit in  sandbox state
                if (_chain_db->get_is_in_sandbox())
                    FC_THROW_EXCEPTION(sandbox_command_forbidden, "in sandbox, this command is forbidden, you cannot call it!");
            ...
            std::map<uint64_t, vector<ForkEntry>> ClientImpl::blockchain_list_forks()const {
                // set limit in  sandbox state
                if (_chain_db->get_is_in_sandbox())
                    FC_THROW_EXCEPTION(sandbox_command_forbidden, "in sandbox, this command is forbidden, you cannot call it!");
            ...
            vector<SlotEntry> ClientImpl::blockchain_get_delegate_slot_entrys(const string& delegate_name, uint64_t limit)const {
                // set limit in  sandbox state
                if (_chain_db->get_is_in_sandbox())
                    FC_THROW_EXCEPTION(sandbox_command_forbidden, "in sandbox, this command is forbidden, you cannot call it!");
            ...
            string ClientImpl::blockchain_get_block_signee(const string& block)const {
				...
				{
					STRING_TO_BLOCKNUM(block);
					return _chain_db->get_block_signee(block_num).name;
				}
            ...
            vector<EventOperation> ClientImpl::blockchain_get_events(const std::string& block, const thinkyoung::blockchain::TransactionIdType& trx_id) const {
                vector<EventOperation> ops;
                
                if (_chain_db) {
					STRING_TO_BLOCKNUM(block);
					ops = _chain_db->get_events(block_num, trx_id);
                }
