...
namespace thinkyoung {
    namespace rpc_stubs {
        ...
        std::vector<thinkyoung::blockchain::AlpTrxidBalance> CommonApiClient::blockchain_get_alp_account_balance_entry(uint64_t block_num)
        {
        ...
        uint32_t CommonApiClient::blockchain_get_total_seednode() const
        {
            ilog("received RPC call: blockchain_get_total_seednode()", );
            thinkyoung::api::GlobalApiLogger* glog = thinkyoung::api::GlobalApiLogger::get_instance();
            uint64_t call_id = 0;
            fc::variants args;
            if( glog != NULL )
            {
                call_id = glog->log_call_started( this, "blockchain_get_total_seednode", args );
            }
            ...
            struct scope_exit
            {
                fc::time_point start_time;
                scope_exit() : start_time(fc::time_point::now()) {}
                ~scope_exit() { dlog("RPC call blockchain_get_total_seednode finished in ${time} ms", ("time", (fc::time_point::now() - start_time).count() / 1000)); }
            } execution_time_logger;
            try
            {
                uint32_t result =             get_impl()->blockchain_get_total_seednode();
                if( call_id != 0 )
                    glog->log_call_finished( call_id, this, "blockchain_get_total_seednode", args, fc::variant(result) );
        ...
        uint32_t CommonApiClient::blockchain_get_total_status_nodes() const
        {
            ilog("received RPC call: blockchain_get_total_status_nodes()", );
            thinkyoung::api::GlobalApiLogger* glog = thinkyoung::api::GlobalApiLogger::get_instance();
            uint64_t call_id = 0;
            fc::variants args;
            if( glog != NULL )
            {
                call_id = glog->log_call_started( this, "blockchain_get_total_status_nodes", args );
            }
            ...
            struct scope_exit
            {
                fc::time_point start_time;
                scope_exit() : start_time(fc::time_point::now()) {}
                ~scope_exit() { dlog("RPC call blockchain_get_total_status_nodes finished in ${time} ms", ("time", (fc::time_point::now() - start_time).count() / 1000)); }
            } execution_time_logger;
            try
            {
                uint32_t result = get_impl()->blockchain_get_total_status_nodes();
                if( call_id != 0 )
                    glog->log_call_finished( call_id, this, "blockchain_get_total_status_nodes", args, fc::variant(result) );
        ...
        uint32_t CommonApiClient::status_node_get_index() const
        {
            ilog("received RPC call: status_node_get_index()", );
            thinkyoung::api::GlobalApiLogger* glog = thinkyoung::api::GlobalApiLogger::get_instance();
            uint64_t call_id = 0;
            fc::variants args;
            if( glog != NULL )
            {
                call_id = glog->log_call_started( this, "status_node_get_index", args );
            }
            ...
            struct scope_exit
            {
                fc::time_point start_time;
                scope_exit() : start_time(fc::time_point::now()) {}
                ~scope_exit() { dlog("RPC call status_node_get_index finished in ${time} ms", ("time", (fc::time_point::now() - start_time).count() / 1000)); }
            } execution_time_logger;
            try
            {
                uint32_t result = get_impl()->status_node_get_index();
                if( call_id != 0 )
                    glog->log_call_finished( call_id, this, "status_node_get_index", args, fc::variant(result) );
        ...
        uint32_t CommonApiClient::status_node_unlocked_wallets_count() const
        {
            ilog("received RPC call: status_node_unlocked_wallets_count()", );
            thinkyoung::api::GlobalApiLogger* glog = thinkyoung::api::GlobalApiLogger::get_instance();
            uint64_t call_id = 0;
            fc::variants args;
            if( glog != NULL )
            {
                call_id = glog->log_call_started( this, "status_node_unlocked_wallets_count", args );
            }
            ...
            struct scope_exit
            {
                fc::time_point start_time;
                scope_exit() : start_time(fc::time_point::now()) {}
                ~scope_exit() { dlog("RPC call status_node_unlocked_wallets_count finished in ${time} ms", ("time", (fc::time_point::now() - start_time).count() / 1000)); }
            } execution_time_logger;
            try
            {
                uint32_t result = get_impl()->status_node_unlocked_wallets_count();
                if( call_id != 0 )
                    glog->log_call_finished( call_id, this, "status_node_unlocked_wallets_count", args, fc::variant(result) );
        ...
        uint64_t CommonApiClient::blockchain_get_block_count(const string& nodeindex) const
        {
            ...
            try
            {
                uint64_t result =             get_impl()->blockchain_get_block_count(nodeindex);
                if( call_id != 0 )
		...
		uint64_t CommonApiClient::status_node_close_wallet(const string& index) const
        {
            ilog("received RPC call: status_node_close_wallet()", );
            thinkyoung::api::GlobalApiLogger* glog = thinkyoung::api::GlobalApiLogger::get_instance();
            uint64_t call_id = 0;
            fc::variants args;
            if( glog != NULL )
            {
                call_id = glog->log_call_started( this, "status_node_close_wallet", args );
            }
            ...
            struct scope_exit
            {
                fc::time_point start_time;
                scope_exit() : start_time(fc::time_point::now()) {}
                ~scope_exit() { dlog("RPC call status_node_close_wallet finished in ${time} ms", ("time", (fc::time_point::now() - start_time).count() / 1000)); }
            } execution_time_logger;
            try
            {
				uint64_t result = get_impl()->status_node_close_wallet(index);
                if( call_id != 0 )
                    glog->log_call_finished( call_id, this, "status_node_close_wallet", args, NULL );
				return result;
			}
            FC_RETHROW_EXCEPTIONS(warn, "")
        }

		uint64_t CommonApiClient::status_node_lock_wallet(const string& index) const
        {
            ilog("received RPC call: status_node_lock_wallet()", );
            thinkyoung::api::GlobalApiLogger* glog = thinkyoung::api::GlobalApiLogger::get_instance();
            uint64_t call_id = 0;
            fc::variants args;
            if( glog != NULL )
            {
                call_id = glog->log_call_started( this, "status_node_lock_wallet", args );
            }
            ...
            struct scope_exit
            {
                fc::time_point start_time;
                scope_exit() : start_time(fc::time_point::now()) {}
                ~scope_exit() { dlog("RPC call status_node_lock_wallet finished in ${time} ms", ("time", (fc::time_point::now() - start_time).count() / 1000)); }
            } execution_time_logger;
            try
            {
				uint64_t result = get_impl()->status_node_lock_wallet(index);
                if( call_id != 0 )
                    glog->log_call_finished( call_id, this, "status_node_lock_wallet", args, NULL );
				return result;
			}
            FC_RETHROW_EXCEPTIONS(warn, "")
        }
		...
		thinkyoung::blockchain::oFullBlockEntry CommonApiClient::blockchain_get_block(const std::string& block) const
		{
			...
			try
			{
				thinkyoung::blockchain::oFullBlockEntry result = get_impl()->blockchain_get_block(block);
				if (call_id != 0)
		...
		thinkyoung::blockchain::oFullBlockEntry CommonApiClient::blockchain_get_fullblock(const std::string& block) const
		{
			ilog("received RPC call: blockchain_get_fullblock(${block})", ("block", block));
			thinkyoung::api::GlobalApiLogger* glog = thinkyoung::api::GlobalApiLogger::get_instance();
			uint64_t call_id = 0;
			fc::variants args;
			if (glog != NULL)
			{
				args.push_back(fc::variant(block));
				call_id = glog->log_call_started(this, "blockchain_get_fullblock", args);
			}
			...
			struct scope_exit
			{
				fc::time_point start_time;
				scope_exit() : start_time(fc::time_point::now()) {}
				~scope_exit() { dlog("RPC call blockchain_get_fullblock finished in ${time} ms", ("time", (fc::time_point::now() - start_time).count() / 1000)); }
			} execution_time_logger;
			try
			{
				thinkyoung::blockchain::oFullBlockEntry result = get_impl()->blockchain_get_fullblock(block);
				if (call_id != 0)
					glog->log_call_finished(call_id, this, "blockchain_get_fullblock", args, fc::variant(result));
		...
		std::vector<oFullBlockEntry> CommonApiClient::blockchain_get_fullblock_list(const std::string& block, const uint32_t limit) const
		{
			ilog("received RPC call: blockchain_get_fullblock_list(${block}-${limit})", ("block", block)("limit", limit));
			thinkyoung::api::GlobalApiLogger* glog = thinkyoung::api::GlobalApiLogger::get_instance();
			uint64_t call_id = 0;
			fc::variants args;
			if (glog != NULL)
			{
				args.push_back(fc::variant(block));
				args.push_back(fc::variant(limit));
				call_id = glog->log_call_started(this, "blockchain_get_fullblock_list", args);
			}
			...
			struct scope_exit
			{
				fc::time_point start_time;
				scope_exit() : start_time(fc::time_point::now()) {}
				~scope_exit() { dlog("RPC call blockchain_get_fullblock_list finished in ${time} ms", ("time", (fc::time_point::now() - start_time).count() / 1000)); }
			} execution_time_logger;
			try
			{
				std::vector<oFullBlockEntry> result = get_impl()->blockchain_get_fullblock_list(block, limit);
				if (call_id != 0)
					glog->log_call_finished(call_id, this, "blockchain_get_fullblock_list", args, fc::variant(result));
        ...
        std::vector<thinkyoung::blockchain::AccountEntry> CommonApiClient::blockchain_list_active_delegates(uint64_t first /* = fc::json::from_string("0").as<uint32_t>() */, uint64_t count /* = fc::json::from_string("20").as<uint32_t>() */) const
        {
        ...
        std::vector<thinkyoung::blockchain::AccountEntry> CommonApiClient::blockchain_list_delegates(uint64_t first /* = fc::json::from_string("0").as<uint32_t>() */, uint64_t count /* = fc::json::from_string("20").as<uint32_t>() */) const
        {
        ...
        std::vector<thinkyoung::blockchain::BlockEntry> CommonApiClient::blockchain_list_blocks(uint64_t max_block_num /* = fc::json::from_string("-1").as<uint32_t>() */, uint64_t limit /* = fc::json::from_string("20").as<uint32_t>() */)
        {
        ...
        std::vector<std::string> CommonApiClient::blockchain_list_missing_block_delegates(uint64_t block_number)
        {
        ...
        std::string CommonApiClient::blockchain_export_fork_graph(uint64_t start_block /* = fc::json::from_string("1").as<uint32_t>() */, uint64_t end_block /* = fc::json::from_string("-1").as<uint32_t>() */, const thinkyoung::blockchain::FilePath& filename /* = fc::json::from_string("\"\"").as<thinkyoung::blockchain::FilePath>() */) const
        {
        ...
        std::map<uint64_t, std::vector<thinkyoung::blockchain::ForkEntry>> CommonApiClient::blockchain_list_forks() const
        {
            ...
            try
            {
                std::map<uint64_t, std::vector<thinkyoung::blockchain::ForkEntry>> result =             get_impl()->blockchain_list_forks();
                if( call_id != 0 )
        ...
        std::vector<thinkyoung::blockchain::SlotEntry> CommonApiClient::blockchain_get_delegate_slot_entrys(const std::string& delegate_name, uint64_t limit /* = fc::json::from_string("\"10\"").as<uint32_t>() */) const
        {
        ...
        std::vector<thinkyoung::blockchain::EventOperation> CommonApiClient::blockchain_get_events(const std::string& block_number, const thinkyoung::blockchain::TransactionIdType& trx_id) const
        {
        ...
        std::vector<fc::variant_object> CommonApiClient::status_nodes_info(bool not_firewalled /* = fc::json::from_string("false").as<bool>() */) const
        {
            ilog("received RPC call: status_nodes_info(${not_firewalled})", ("not_firewalled", not_firewalled));
            thinkyoung::api::GlobalApiLogger* glog = thinkyoung::api::GlobalApiLogger::get_instance();
            uint64_t call_id = 0;
            fc::variants args;
            if( glog != NULL )
            {
                args.push_back( fc::variant(not_firewalled) );
                call_id = glog->log_call_started( this, "status_nodes_info", args );
            }
            ...
            struct scope_exit
            {
                fc::time_point start_time;
                scope_exit() : start_time(fc::time_point::now()) {}
                ~scope_exit() { dlog("RPC call status_nodes_info finished in ${time} ms", ("time", (fc::time_point::now() - start_time).count() / 1000)); }
            } execution_time_logger;
            try
            {
                std::vector<fc::variant_object> result = get_impl()->status_nodes_info(not_firewalled);
                if( call_id != 0 )
                    glog->log_call_finished( call_id, this, "status_nodes_info", args, fc::variant(result) );
        ...
        std::vector<fc::variant_object> CommonApiClient::status_node_unlocked_wallets_list(bool not_firewalled /* = fc::json::from_string("false").as<bool>() */) const
        {
            ilog("received RPC call: status_node_unlocked_wallets_list(${not_firewalled})", ("not_firewalled", not_firewalled));
            thinkyoung::api::GlobalApiLogger* glog = thinkyoung::api::GlobalApiLogger::get_instance();
            uint64_t call_id = 0;
            fc::variants args;
            if( glog != NULL )
            {
                args.push_back( fc::variant(not_firewalled) );
                call_id = glog->log_call_started( this, "status_node_unlocked_wallets_list", args );
            }
            ...
            struct scope_exit
            {
                fc::time_point start_time;
                scope_exit() : start_time(fc::time_point::now()) {}
                ~scope_exit() { dlog("RPC call status_node_unlocked_wallets_list finished in ${time} ms", ("time", (fc::time_point::now() - start_time).count() / 1000)); }
            } execution_time_logger;
            try
            {
                std::vector<fc::variant_object> result = get_impl()->status_node_unlocked_wallets_list(not_firewalled);
                if( call_id != 0 )
                    glog->log_call_finished( call_id, this, "status_node_unlocked_wallets_list", args, fc::variant(result) );
    ...
    if( glog->obscure_passwords() )
      args.push_back( fc::variant("*********") );
        ...
        std::vector<thinkyoung::wallet::PrettyTransaction> CommonApiClient::wallet_account_transaction_history(const std::string& account_name /* = fc::json::from_string("\"\"").as<std::string>() */, const std::string& asset_symbol /* = fc::json::from_string("\"\"").as<std::string>() */, int64_t limit /* = fc::json::from_string("0").as<int64_t>() */, uint64_t start_block_num /* = fc::json::from_string("0").as<uint64_t>() */, uint64_t end_block_num /* = fc::json::from_string("-1").as<uint64_t>() */) const
        {
        ...
        std::vector<thinkyoung::wallet::PrettyTransaction> CommonApiClient::wallet_transaction_history_splite(const std::string& account_name /* = fc::json::from_string("\"\"").as<std::string>() */, const std::string& asset_symbol /* = fc::json::from_string("\"\"").as<std::string>() */, int64_t limit /* = fc::json::from_string("0").as<int64_t>() */, int32_t transaction_type /* = fc::json::from_string("\"2\"").as<int32_t>() */) const
        {
    ...
    if( glog->obscure_passwords() )
      args.push_back( fc::variant("*********") );
        ...
        void CommonApiClient::wallet_rescan_blockchain(uint64_t start_block_num /* = fc::json::from_string("0").as<uint32_t>() */, uint32_t limit /* = fc::json::from_string("-1").as<uint32_t>() */)
        {
