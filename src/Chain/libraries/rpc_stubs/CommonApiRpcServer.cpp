...
namespace thinkyoung {
    namespace rpc_stubs {
        ...
        fc::variant CommonApiRpcServer::blockchain_get_alp_account_balance_entry_positional(fc::rpc::json_connection* json_connection, const fc::variants& parameters)
        {
            ...
            uint64_t block_num = parameters[0].as<uint64_t>();
        ...
        fc::variant CommonApiRpcServer::blockchain_get_alp_account_balance_entry_named(fc::rpc::json_connection* json_connection, const fc::variant_object& parameters)
        {
            ...
            uint64_t block_num = parameters["block_num"].as<uint64_t>();
        ...
        fc::variant CommonApiRpcServer::blockchain_get_total_seednode_positional(fc::rpc::json_connection* json_connection, const fc::variants& parameters)
        {
            uint32_t result = get_client()->blockchain_get_total_seednode();
            return fc::variant(result);
        }

        fc::variant CommonApiRpcServer::blockchain_get_total_seednode_named(fc::rpc::json_connection* json_connection, const fc::variant_object& parameters)
        {
            bool result = get_client()->blockchain_get_total_seednode();
            return fc::variant(result);
        }

        fc::variant CommonApiRpcServer::blockchain_get_total_status_nodes_positional(fc::rpc::json_connection* json_connection, const fc::variants& parameters)
        {
            uint32_t result = get_client()->blockchain_get_total_status_nodes();
            return fc::variant(result);
        }

        fc::variant CommonApiRpcServer::blockchain_get_total_status_nodes_named(fc::rpc::json_connection* json_connection, const fc::variant_object& parameters)
        {
            bool result = get_client()->blockchain_get_total_status_nodes();
            return fc::variant(result);
        }

        fc::variant CommonApiRpcServer::status_node_get_index_positional(fc::rpc::json_connection* json_connection, const fc::variants& parameters)
        {
            uint32_t result = get_client()->status_node_get_index();
            return fc::variant(result);
        }

        fc::variant CommonApiRpcServer::status_node_get_index_named(fc::rpc::json_connection* json_connection, const fc::variant_object& parameters)
        {
            bool result = get_client()->status_node_get_index();
            return fc::variant(result);
        }

        fc::variant CommonApiRpcServer::status_node_unlocked_wallets_count_positional(fc::rpc::json_connection* json_connection, const fc::variants& parameters)
        {
            uint32_t result = get_client()->status_node_unlocked_wallets_count();
            return fc::variant(result);
        }

        fc::variant CommonApiRpcServer::status_node_unlocked_wallets_count_named(fc::rpc::json_connection* json_connection, const fc::variant_object& parameters)
        {
            bool result = get_client()->status_node_unlocked_wallets_count();
            return fc::variant(result);
        }
        ...
        fc::variant CommonApiRpcServer::blockchain_get_block_count_positional(fc::rpc::json_connection* json_connection, const fc::variants& parameters)
        {
			...
			if (parameters.size() > 0) nodeindex = parameters[0].as<std::string>();
			
			uint64_t result = get_client()->blockchain_get_block_count(nodeindex);
            return fc::variant(result);
        }
        ...
        fc::variant CommonApiRpcServer::blockchain_get_block_count_named(fc::rpc::json_connection* json_connection, const fc::variant_object& parameters)
        {
            // this method has no prerequisites
			std::string nodeindex = "";
			if (parameters.contains("nodeindex")) nodeindex = parameters["nodeindex"].as<std::string>();

            uint64_t result = get_client()->blockchain_get_block_count(nodeindex);
            return fc::variant(result);
        }

		fc::variant CommonApiRpcServer::status_node_close_wallet_positional(fc::rpc::json_connection* json_connection, const fc::variants& parameters)
        {
            // this method has no prerequisites
			std::string index = "0";
			if (parameters.size() > 0) index = parameters[0].as<std::string>();
			
			uint64_t result = get_client()->status_node_close_wallet(index);
			return fc::variant(result);
		}

		fc::variant CommonApiRpcServer::status_node_close_wallet_named(fc::rpc::json_connection* json_connection, const fc::variant_object& parameters)
        {
            // this method has no prerequisites
			std::string index = "0";
			if (parameters.contains("index")) index = parameters["index"].as<std::string>();

			uint64_t result = get_client()->status_node_close_wallet(index);
			return fc::variant(result);
		}

		fc::variant CommonApiRpcServer::status_node_lock_wallet_positional(fc::rpc::json_connection* json_connection, const fc::variants& parameters)
        {
            // this method has no prerequisites
			std::string index = "0";
			if (parameters.size() > 0) index = parameters[0].as<std::string>();
			
			uint64_t result = get_client()->status_node_lock_wallet(index);
			return fc::variant(result);
		}

		fc::variant CommonApiRpcServer::status_node_lock_wallet_named(fc::rpc::json_connection* json_connection, const fc::variant_object& parameters)
        {
            // this method has no prerequisites
			std::string index = "0";
			if (parameters.contains("index")) index = parameters["index"].as<std::string>();
		...
		fc::variant CommonApiRpcServer::blockchain_get_block_positional(fc::rpc::json_connection* json_connection, const fc::variants& parameters)
		{
			...
			thinkyoung::blockchain::oFullBlockEntry result = get_client()->blockchain_get_block(block);
			return fc::variant(result);
		}

		fc::variant CommonApiRpcServer::blockchain_get_fullblock_positional(fc::rpc::json_connection* json_connection, const fc::variants& parameters)
		{
			// this method has no prerequisites
			...
			thinkyoung::blockchain::oFullBlockEntry result = get_client()->blockchain_get_fullblock(block);
			return fc::variant(result);
		}

		fc::variant CommonApiRpcServer::blockchain_get_fullblock_list_positional(fc::rpc::json_connection* json_connection, const fc::variants& parameters)
		{
			// this method has no prerequisites
			...
			std::string block = parameters[0].as<std::string>();
			uint32_t limit = 1000; 
			if(parameters.size() >= 2) limit = parameters[1].as<uint32_t>();

			std::vector<thinkyoung::blockchain::oFullBlockEntry> result = get_client()->blockchain_get_fullblock_list(block, limit);
			return fc::variant(result);
		}
		...
		fc::variant CommonApiRpcServer::blockchain_get_fullblock_named(fc::rpc::json_connection* json_connection, const fc::variant_object& parameters)
		{
			// this method has no prerequisites
		...
		fc::variant CommonApiRpcServer::blockchain_get_fullblock_list_named(fc::rpc::json_connection* json_connection, const fc::variant_object& parameters)
		{
			// this method has no prerequisites
			...
			std::string block = parameters["block"].as<std::string>();
			uint32_t limit = 1000;
			if (parameters.contains("limit") >= 2) limit = parameters["limit"].as<uint32_t>();
        ...
        fc::variant CommonApiRpcServer::blockchain_list_blocks_positional(fc::rpc::json_connection* json_connection, const fc::variants& parameters)
        {
            ...
            uint64_t max_block_num = (parameters.size() <= 0) ?
            (fc::json::from_string("-1").as<uint64_t>()) :
            parameters[0].as<uint64_t>();
            uint64_t limit = (parameters.size() <= 1) ?
            (fc::json::from_string("20").as<uint64_t>()) :
            parameters[1].as<uint64_t>();
        ...
        fc::variant CommonApiRpcServer::blockchain_list_blocks_named(fc::rpc::json_connection* json_connection, const fc::variant_object& parameters)
        {
            ...
            uint64_t max_block_num = parameters.contains("max_block_num") ?
                (fc::json::from_string("-1").as<uint64_t>()) :
                parameters["max_block_num"].as<uint64_t>();
            uint64_t limit = parameters.contains("limit") ?
                (fc::json::from_string("20").as<uint64_t>()) :
                parameters["limit"].as<uint64_t>();
        ...
        fc::variant CommonApiRpcServer::blockchain_list_missing_block_delegates_positional(fc::rpc::json_connection* json_connection, const fc::variants& parameters)
        {
            ...
            uint64_t block_number = parameters[0].as<uint64_t>();
        ...
        fc::variant CommonApiRpcServer::blockchain_list_missing_block_delegates_named(fc::rpc::json_connection* json_connection, const fc::variant_object& parameters)
        {
            ...
            uint64_t block_number = parameters["block_number"].as<uint64_t>();
        ...
        fc::variant CommonApiRpcServer::blockchain_export_fork_graph_positional(fc::rpc::json_connection* json_connection, const fc::variants& parameters)
        {
            ...
            uint64_t start_block = (parameters.size() <= 0) ?
            (fc::json::from_string("1").as<uint64_t>()) :
            parameters[0].as<uint32_t>();
            uint64_t end_block = (parameters.size() <= 1) ?
            (fc::json::from_string("-1").as<uint64_t>()) :
            parameters[1].as<uint32_t>();
        ...
        fc::variant CommonApiRpcServer::blockchain_export_fork_graph_named(fc::rpc::json_connection* json_connection, const fc::variant_object& parameters)
        {
            ...
            uint64_t start_block = parameters.contains("start_block") ?
                (fc::json::from_string("1").as<uint64_t>()) :
                parameters["start_block"].as<uint64_t>();
            uint64_t end_block = parameters.contains("end_block") ?
                (fc::json::from_string("-1").as<uint64_t>()) :
                parameters["end_block"].as<uint64_t>();
        ...
        fc::variant CommonApiRpcServer::blockchain_list_forks_positional(fc::rpc::json_connection* json_connection, const fc::variants& parameters)
        {
            ...
            std::map<uint64_t, std::vector<thinkyoung::blockchain::ForkEntry>> result = get_client()->blockchain_list_forks();
            return fc::variant(result);
        }
        ...
        fc::variant CommonApiRpcServer::blockchain_get_delegate_slot_entrys_positional(fc::rpc::json_connection* json_connection, const fc::variants& parameters)
        {
            ...
            uint64_t limit = (parameters.size() <= 1) ?
            (fc::json::from_string("\"10\"").as<uint64_t>()) :
            parameters[1].as<uint64_t>();
        ...
        fc::variant CommonApiRpcServer::blockchain_get_events_positional(fc::rpc::json_connection* json_connection, const fc::variants& parameters)
        {
            ...
            if (parameters.size() <= 0)
                FC_THROW_EXCEPTION(fc::invalid_arg_exception, "missing required parameter 1 (block_number)");
			std::string block_number = parameters[0].as<std::string>();
            if (parameters.size() <= 1)
        ...
        fc::variant CommonApiRpcServer::blockchain_get_events_named(fc::rpc::json_connection* json_connection, const fc::variant_object& parameters)
        {
            ...
            if (!parameters.contains("block_number"))
                FC_THROW_EXCEPTION(fc::invalid_arg_exception, "missing required parameter 'block_number'");
			std::string block_number = parameters["block_number"].as<std::string>();
        ...
        fc::variant CommonApiRpcServer::status_nodes_info_positional(fc::rpc::json_connection* json_connection, const fc::variants& parameters)
        {
            // check all of this method's prerequisites
            verify_json_connection_is_authenticated(json_connection);
            // done checking prerequisites
            ...
            std::vector<fc::variant_object> result = get_client()->status_nodes_info(not_firewalled);
            return fc::variant(result);
        }

        fc::variant CommonApiRpcServer::status_nodes_info_named(fc::rpc::json_connection* json_connection, const fc::variant_object& parameters)
        {
            // check all of this method's prerequisites
            verify_json_connection_is_authenticated(json_connection);
            // done checking prerequisites
        ...
        fc::variant CommonApiRpcServer::status_node_unlocked_wallets_list_positional(fc::rpc::json_connection* json_connection, const fc::variants& parameters)
        {
            // check all of this method's prerequisites
            verify_json_connection_is_authenticated(json_connection);
            // done checking prerequisites
            ...
            std::vector<fc::variant_object> result = get_client()->status_node_unlocked_wallets_list(not_firewalled);
            return fc::variant(result);
        }

        fc::variant CommonApiRpcServer::status_node_unlocked_wallets_list_named(fc::rpc::json_connection* json_connection, const fc::variant_object& parameters)
        {
            // check all of this method's prerequisites
            verify_json_connection_is_authenticated(json_connection);
            // done checking prerequisites
        ...
        fc::variant CommonApiRpcServer::wallet_account_transaction_history_positional(fc::rpc::json_connection* json_connection, const fc::variants& parameters)
        {
            ...
            (fc::json::from_string("\"\"").as<std::string>()) :
            parameters[1].as<std::string>();
            int64_t limit = (parameters.size() <= 2) ?
            (fc::json::from_string("0").as<int64_t>()) :
            parameters[2].as<int64_t>();
            uint64_t start_block_num = (parameters.size() <= 3) ?
            (fc::json::from_string("0").as<uint64_t>()) :
            parameters[3].as<uint64_t>();
            uint64_t end_block_num = (parameters.size() <= 4) ?
            (fc::json::from_string("-1").as<uint64_t>()) :
            parameters[4].as<uint64_t>();
        ...
        fc::variant CommonApiRpcServer::wallet_account_transaction_history_named(fc::rpc::json_connection* json_connection, const fc::variant_object& parameters)
        {
            ...
            int64_t limit = parameters.contains("limit") ?
                (fc::json::from_string("0").as<int64_t>()) :
                parameters["limit"].as<int64_t>();
            uint64_t start_block_num = parameters.contains("start_block_num") ?
                (fc::json::from_string("0").as<uint64_t>()) :
                parameters["start_block_num"].as<uint64_t>();
            uint64_t end_block_num = parameters.contains("end_block_num") ?
                (fc::json::from_string("-1").as<uint64_t>()) :
                parameters["end_block_num"].as<uint64_t>();
        ...
        void CommonApiRpcServer::register_common_api_methods(const fc::rpc::json_connection_ptr& json_connection)
        {
           ...
           // register method blockchain_get_total_seednode
            bound_positional_method = boost::bind(&CommonApiRpcServer::blockchain_get_total_seednode_positional,
                this, capture_con, _1);
            json_connection->add_method("blockchain_get_total_seednode", bound_positional_method);
            json_connection->add_method("seednode", bound_positional_method);
            bound_named_method = boost::bind(&CommonApiRpcServer::blockchain_get_total_seednode_named,
                this, capture_con, _1);
            json_connection->add_named_param_method("blockchain_get_total_seednode", bound_named_method);
            json_connection->add_named_param_method("seednode", bound_named_method);

           // register method blockchain_get_total_status_nodes
            bound_positional_method = boost::bind(&CommonApiRpcServer::blockchain_get_total_status_nodes_positional,
                this, capture_con, _1);
            json_connection->add_method("blockchain_get_total_status_nodes", bound_positional_method);
            json_connection->add_method("statusnode", bound_positional_method);
            bound_named_method = boost::bind(&CommonApiRpcServer::blockchain_get_total_status_nodes_named,
                this, capture_con, _1);
            json_connection->add_named_param_method("blockchain_get_total_status_nodes", bound_named_method);
            json_connection->add_named_param_method("statusnode", bound_named_method);

           // register method status_node_get_index
            bound_positional_method = boost::bind(&CommonApiRpcServer::status_node_get_index_positional,
                this, capture_con, _1);
            json_connection->add_method("status_node_get_index", bound_positional_method);
            json_connection->add_method("statusindex", bound_positional_method);
            bound_named_method = boost::bind(&CommonApiRpcServer::status_node_get_index_named,
                this, capture_con, _1);
            json_connection->add_named_param_method("status_node_get_index", bound_named_method);
            json_connection->add_named_param_method("statusindex", bound_named_method);

           // register method status_node_unlocked_wallets_count
            bound_positional_method = boost::bind(&CommonApiRpcServer::status_node_unlocked_wallets_count_positional,
                this, capture_con, _1);
            json_connection->add_method("status_node_unlocked_wallets_count", bound_positional_method);
            json_connection->add_method("unlockedcount", bound_positional_method);
            bound_named_method = boost::bind(&CommonApiRpcServer::status_node_unlocked_wallets_count_named,
                this, capture_con, _1);
            json_connection->add_named_param_method("status_node_unlocked_wallets_count", bound_named_method);
            json_connection->add_named_param_method("unlockedcount", bound_named_method);
           ...
           // register method blockchain_get_block_count
            bound_positional_method = boost::bind(&CommonApiRpcServer::status_node_close_wallet_positional,
                this, capture_con, _1);
            json_connection->add_method("status_node_close_wallet", bound_positional_method);
            bound_named_method = boost::bind(&CommonApiRpcServer::status_node_close_wallet_named,
                this, capture_con, _1);
            json_connection->add_named_param_method("status_node_close_wallet", bound_named_method);

           // register method blockchain_get_block_count
            bound_positional_method = boost::bind(&CommonApiRpcServer::status_node_lock_wallet_positional,
                this, capture_con, _1);
            json_connection->add_method("status_node_lock_wallet", bound_positional_method);
            bound_named_method = boost::bind(&CommonApiRpcServer::status_node_lock_wallet_named,
                this, capture_con, _1);
            json_connection->add_named_param_method("status_node_lock_wallet", bound_named_method);
           ...
           // register method blockchain_get_block
			//bound_positional_method = boost::bind(&CommonApiRpcServer::blockchain_get_fullblock_positional,
			//	this, capture_con, _1);
			...
			//bound_named_method = boost::bind(&CommonApiRpcServer::blockchain_get_fullblock_named,
			//	this, capture_con, _1);
			...
			// register method blockchain_get_fullblock
			bound_positional_method = boost::bind(&CommonApiRpcServer::blockchain_get_fullblock_positional,
				this, capture_con, _1);
			json_connection->add_method("blockchain_get_block", bound_positional_method);
			json_connection->add_method("get_block", bound_positional_method);
			...
			json_connection->add_method("blockchain_get_blockhash", bound_positional_method);
			json_connection->add_method("getblockhash", bound_positional_method);
			json_connection->add_method("blockchain_get_fullblock", bound_positional_method);
			json_connection->add_method("get_fullblock", bound_positional_method);
			json_connection->add_method("getfullblock", bound_positional_method);
			json_connection->add_method("blockchain_get_fullblock_hash", bound_positional_method);
			json_connection->add_method("blockchain_get_fullblockhash", bound_positional_method);
			json_connection->add_method("getfullblockhash", bound_positional_method);
			...
			json_connection->add_named_param_method("blockchain_get_blockhash", bound_named_method);
			json_connection->add_named_param_method("getblockhash", bound_named_method);
			json_connection->add_named_param_method("blockchain_get_fullblock", bound_named_method);
			json_connection->add_named_param_method("get_fullblock", bound_named_method);
			json_connection->add_named_param_method("getfullblock", bound_named_method);
			json_connection->add_named_param_method("blockchain_get_fullblock_hash", bound_named_method);
			json_connection->add_named_param_method("blockchain_get_fullblockhash", bound_named_method);
			json_connection->add_named_param_method("getfullblockhash", bound_named_method);

			// register method blockchain_get_fullblock_list
			bound_positional_method = boost::bind(&CommonApiRpcServer::blockchain_get_fullblock_list_positional,
				this, capture_con, _1);
			json_connection->add_method("blockchain_get_fullblock_list", bound_positional_method);
			json_connection->add_method("get_fullblock_list", bound_positional_method);
			json_connection->add_method("getfullblocklist", bound_positional_method);
			json_connection->add_method("blockchain_get_fullblock_list_hash", bound_positional_method);
			json_connection->add_method("blockchain_get_fullblocklisthash", bound_positional_method);
			json_connection->add_method("getfullblocklisthash", bound_positional_method);

			bound_named_method = boost::bind(&CommonApiRpcServer::blockchain_get_fullblock_list_named,
				this, capture_con, _1);
			json_connection->add_named_param_method("blockchain_get_fullblock_list", bound_named_method);
			json_connection->add_named_param_method("get_fullblock_list", bound_named_method);
			json_connection->add_named_param_method("getfullblocklist", bound_named_method);
			json_connection->add_named_param_method("blockchain_get_fullblock_list_hash", bound_named_method);
			json_connection->add_named_param_method("blockchain_get_fullblocklisthash", bound_named_method);
			json_connection->add_named_param_method("getfullblocklisthash", bound_named_method);
           ...
           // register method network_get_peer_info
			...
			json_connection->add_method("peerinfo", bound_positional_method);
			...
			json_connection->add_named_param_method("peerinfo", bound_named_method);

           // register method status_nodes_info
            bound_positional_method = boost::bind(&CommonApiRpcServer::status_nodes_info_positional,
                this, capture_con, _1);
			json_connection->add_method("status_nodes_info", bound_positional_method);
			json_connection->add_method("statusinfo", bound_positional_method);
            bound_named_method = boost::bind(&CommonApiRpcServer::status_nodes_info_named,
                this, capture_con, _1);
			json_connection->add_named_param_method("status_nodes_info", bound_named_method);
			json_connection->add_named_param_method("statusinfo", bound_named_method);

           // register method status_nodes_info
            bound_positional_method = boost::bind(&CommonApiRpcServer::status_node_unlocked_wallets_list_positional,
                this, capture_con, _1);
			json_connection->add_method("status_node_unlocked_wallets_list", bound_positional_method);
			json_connection->add_method("unlockedlist", bound_positional_method);
            bound_named_method = boost::bind(&CommonApiRpcServer::status_node_unlocked_wallets_list_named,
                this, capture_con, _1);
			json_connection->add_named_param_method("status_node_unlocked_wallets_list", bound_named_method);
			json_connection->add_named_param_method("unlockedlist", bound_named_method);
           ...
           // register method network_list_potential_peers
			...
			json_connection->add_method("listpeers", bound_positional_method);
			...
			json_connection->add_named_param_method("list_peers", bound_named_method);
        ...
        void CommonApiRpcServer::register_common_api_method_metadata()
        {
            ...
            {
                // register method blockchain_get_total_seednode
                thinkyoung::api::MethodData blockchain_get_total_seednode_method_metadata{ "blockchain_get_total_seednode", nullptr,
                    /* description */ "Returns the number of seednode connected currently.",
                    /* returns */ "uint32_t",
                    /* params: */{                    },
                    /* prerequisites */ (thinkyoung::api::MethodPrerequisites) 0,
                    /* detailed description */ "Returns the number of seednode connected currently.\n\nParameters:\n  (none)\n\nReturns:\n  uint32_t\n",
                    /* aliases */ {"seednode"}, false};
                store_method_metadata(blockchain_get_total_seednode_method_metadata);
            }

            {
                // register method blockchain_get_total_status_nodes
                thinkyoung::api::MethodData blockchain_get_total_status_nodes_method_metadata{ "blockchain_get_total_status_nodes", nullptr,
                    /* description */ "Returns the number of status nodes connected currently.",
                    /* returns */ "uint32_t",
                    /* params: */{                    },
                    /* prerequisites */ (thinkyoung::api::MethodPrerequisites) 0,
                    /* detailed description */ "Returns the number of status nodes connected currently.\n\nParameters:\n  (none)\n\nReturns:\n  uint32_t\n",
                    /* aliases */ {"statusnodes"}, false};
                store_method_metadata(blockchain_get_total_status_nodes_method_metadata);
            }

            {
                // register method status_node_get_index
                thinkyoung::api::MethodData status_node_get_index_method_metadata{ "status_node_get_index", nullptr,
                    /* description */ "Returns the index of this status node.",
                    /* returns */ "uint32_t",
                    /* params: */{                    },
                    /* prerequisites */ (thinkyoung::api::MethodPrerequisites) 0,
                    /* detailed description */ "Returns the index of this status node.\n\nParameters:\n  (none)\n\nReturns:\n  uint32_t\n",
                    /* aliases */ {"statusindex"}, false};
                store_method_metadata(status_node_get_index_method_metadata);
            }

            {
                // register method status_node_unlocked_wallets_count
                thinkyoung::api::MethodData status_node_unlocked_wallets_count_method_metadata{ "status_node_unlocked_wallets_count", nullptr,
                    /* description */ "Returns the number of unlocked wallets currently.",
                    /* returns */ "uint32_t",
                    /* params: */{                    },
                    /* prerequisites */ (thinkyoung::api::MethodPrerequisites) 0,
                    /* detailed description */ "Returns the number of unlocked wallets currently.\n\nParameters:\n  (none)\n\nReturns:\n  uint32_t\n",
                    /* aliases */ {"unlockedcount"}, false};
                store_method_metadata(status_node_unlocked_wallets_count_method_metadata);
            }

			{
                ...
                thinkyoung::api::MethodData blockchain_get_block_count_method_metadata{ "blockchain_get_block_count", nullptr,
                    ...
                    /* returns */ "uint32_t",
                    /* params: */{
							{"nodeindex", "string", thinkyoung::api::optional_positional, fc::ovariant()}
						},
                    /* prerequisites */ (thinkyoung::api::MethodPrerequisites) 0,
                    /* detailed description */ "Returns the current head block number\n\nParameters:\n  nodeindex (string optional)\n\nReturns:\n  uint32_t\n",
                    /* aliases */ {"blockchain_get_blockcount", "getblockcount"}, true};
                store_method_metadata(blockchain_get_block_count_method_metadata);
            }

			{
                // register method status_node_close_wallet
                thinkyoung::api::MethodData status_node_close_wallet_method_metadata{ "status_node_close_wallet", nullptr,
                    /* description */ "Close forcely unlocked wallet in status node",
                    /* returns */ "uint64_t",
                    /* params: */{
							{"index", "string", thinkyoung::api::optional_positional, fc::ovariant()}
						},
                    /* prerequisites */ (thinkyoung::api::MethodPrerequisites) 0,
                    /* detailed description */ "Close forcely unlocked wallet in status node\n\nParameters:\n  index (string optional)\n\nReturns:\n  uint64_t\n",
                    /* aliases */ {}, true};
                store_method_metadata(status_node_close_wallet_method_metadata);
            }

			{
                // register method status_node_close_wallet
                thinkyoung::api::MethodData status_node_lock_wallet_method_metadata{ "status_node_lock_wallet", nullptr,
                    /* description */ "Lock forcely unlocked wallet in status node",
                    /* returns */ "uint64_t",
                    /* params: */{
							{"index", "string", thinkyoung::api::optional_positional, fc::ovariant()}
						},
                    /* prerequisites */ (thinkyoung::api::MethodPrerequisites) 0,
                    /* detailed description */ "Lock forcely unlocked wallet in status node\n\nParameters:\n  index (string optional)\n\nReturns:\n  uint64_t\n",
                    /* aliases */ {}, true};
                store_method_metadata(status_node_lock_wallet_method_metadata);
            }
			...
			{
				// register method blockchain_get_fullblock
				thinkyoung::api::MethodData blockchain_get_fullblock_method_metadata{ "blockchain_get_fullblock", nullptr,
					/* description */ "Retrieves the full block for the given block number or ID",
					/* returns */ "full_block",
					/* params: */{
						{"block", "string", thinkyoung::api::required_positional, fc::ovariant()}
						  },
					/* prerequisites */ (thinkyoung::api::MethodPrerequisites) 0,
					/* detailed description */ "Retrieves the full block for the given block number or ID\n\nParameters:\n  block (string, required): block number or ID to retrieve\n\nReturns:\n  full_block\n",
					/* aliases */ {"get_fullblock", "getfullblock", "blockchain_get_fullblock_hash", "blockchain_get_fullblockhash", "getfullblockhash"}, true };
				store_method_metadata(blockchain_get_fullblock_method_metadata);
			}

			{
				// register method blockchain_get_fullblock
				thinkyoung::api::MethodData blockchain_get_fullblock_list_method_metadata{ "blockchain_get_fullblock_list", nullptr,
					/* description */ "Retrieves the full block list for the given block number or ID and limit count",
					/* returns */ "full_block_list",
					/* params: */{
						{"block", "string", thinkyoung::api::required_positional, fc::ovariant()},
						{"limit", "uint32_t", thinkyoung::api::optional_positional, fc::ovariant()}
						  },
					/* prerequisites */ (thinkyoung::api::MethodPrerequisites) 0,
					/* detailed description */ "Retrieves the full block list for the given block number or ID and limit count\n\nParameters:\n  block (string, required): block number or ID to retrieve\n  limit (uint32_t, optional): limit count\n\nReturns:\n  full_block_list\n",
					/* aliases */ {"get_fullblock_list", "getfullblocklist", "blockchain_get_fullblock_list_hash", "blockchain_get_fullblocklisthash", "getfullblocklisthash"}, true };
				store_method_metadata(blockchain_get_fullblock_list_method_metadata);
			}
...

            {
                ...
                thinkyoung::api::MethodData blockchain_get_events_method_metadata{ "blockchain_get_events", nullptr,
                    ...
                    /* params: */{
                        {"block_number", "string", thinkyoung::api::required_positional, fc::ovariant()},
...

            {
                ...
                thinkyoung::api::MethodData network_get_peer_info_method_metadata{ "network_get_peer_info", nullptr,
                    ...
                    /* aliases */ {"peerinfo"}, false};
                store_method_metadata(network_get_peer_info_method_metadata);
            }

            {
                // register method network_get_peer_info
                thinkyoung::api::MethodData status_nodes_info_method_metadata{ "status_nodes_info", nullptr,
                    /* description */ "Returns data about connected status nodes",
                    /* returns */ "json_object_array",
                    /* params: */{
                        {"not_firewalled", "bool", thinkyoung::api::optional_positional, fc::variant(fc::json::from_string("false"))}
                    },
                    /* prerequisites */ (thinkyoung::api::MethodPrerequisites) 1,
                    /* detailed description */ "Returns data about connected status nodes\n\nParameters:\n  not_firewalled (bool, optional, defaults to false): true to output only peers not behind a firewall and false otherwise\n\nReturns:\n  json_object_array\n",
                    /* aliases */ {"statusinfo"}, false};
                store_method_metadata(status_nodes_info_method_metadata);
            }
			
            {
                // register method network_get_peer_info
                thinkyoung::api::MethodData status_node_unlocked_wallets_list_method_metadata{ "status_node_unlocked_wallets_list", nullptr,
                    /* description */ "Returns data about unlocked wallets",
                    /* returns */ "json_object_array",
                    /* params: */{
                        {"not_firewalled", "bool", thinkyoung::api::optional_positional, fc::variant(fc::json::from_string("false"))}
                    },
                    /* prerequisites */ (thinkyoung::api::MethodPrerequisites) 1,
                    /* detailed description */ "Returns data about unlocked wallets\n\nParameters:\n  not_firewalled (bool, optional, defaults to false): true to output only peers not behind a firewall and false otherwise\n\nReturns:\n  json_object_array\n",
                    /* aliases */ {"unlockedlist"}, false};
                store_method_metadata(status_node_unlocked_wallets_list_method_metadata);
            }
        ...
        fc::variant CommonApiRpcServer::direct_invoke_positional_method(const std::string& method_name, const fc::variants& parameters)
        {
            ...
            if (method_name == "blockchain_get_total_seednode")
                return blockchain_get_total_seednode_positional(nullptr, parameters);
            if (method_name == "blockchain_get_total_status_nodes")
                return blockchain_get_total_status_nodes_positional(nullptr, parameters);
            if (method_name == "blockchain_get_block_count")
                return blockchain_get_block_count_positional(nullptr, parameters);
            if (method_name == "status_node_close_wallet")
                return status_node_close_wallet_positional(nullptr, parameters);
            if (method_name == "status_node_lock_wallet")
                return status_node_lock_wallet_positional(nullptr, parameters);
			...
			if (method_name == "blockchain_get_fullblock")
				return blockchain_get_fullblock_positional(nullptr, parameters);
			if (method_name == "blockchain_get_fullblock_list")
				return blockchain_get_fullblock_list_positional(nullptr, parameters);
            ...
            if (method_name == "status_node_get_index")
                return status_node_get_index_positional(nullptr, parameters);
            if (method_name == "status_node_unlocked_wallets_count")
                return status_node_unlocked_wallets_count_positional(nullptr, parameters);
            ...
            if (method_name == "status_nodes_info")
                return status_nodes_info_positional(nullptr, parameters);
            if (method_name == "status_node_unlocked_wallets_list")
                return status_node_unlocked_wallets_list_positional(nullptr, parameters);
