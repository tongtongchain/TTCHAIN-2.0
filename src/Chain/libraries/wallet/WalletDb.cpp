...
namespace thinkyoung {
    namespace wallet {
		...
		void WalletDb::store_raw_transaction(const SignedTransaction& trx, const time_point_sec block_timestamp)
		{
			oWalletTransactionEntry transaction_entry = WalletTransactionEntry();
		...
		void WalletDb::store_transaction(const TransactionData& transaction)
		{
			try {
				...
				//transaction_entry->block_num = blockNum;
		...
		void WalletDb::confirm_transaction(const TransactionIdType& transactionId, uint64_t blockNum)
		{
			try {
				if (!is_open()) return;
				//FC_ASSERT(is_open(), "Wallet not open!");
				oWalletTransactionEntry transaction_entry = lookup_transaction(transactionId);
				if (!transaction_entry.valid())
					return;
				...
				transaction_entry->block_num = blockNum;
				store_transaction(*transaction_entry);
			} FC_CAPTURE_AND_RETHROW((transactionId))
		}
		...
		uint64_t WalletDb::get_id()
        {
            try {
				uint64_t id = *(uint64_t*)wallet_master_key->encrypted_key.data();
				return id;
            } FC_CAPTURE_AND_RETHROW()
        }
