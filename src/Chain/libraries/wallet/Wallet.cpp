    ...
    namespace wallet {
        ...
        namespace detail {
            ...
            void WalletImpl::state_changed(const PendingChainStatePtr& state) {
                ...
                /*const auto last_unlocked_scanned_number = self->get_last_scanned_block_numbers();
				vector<uint64_t> indexes = _blockchain->get_index_list();
				bool bNeedUpdate = false;
				for (int i = 0; i < indexes.size(); i++)
				{
					bool bMatch = false;
					BlockNumberType bn = BlockNumberType(indexes[i]);
					for (int j = 0; j < last_unlocked_scanned_number.size(); j++)
					{
						BlockNumberType bn1 = BlockNumberType(last_unlocked_scanned_number[j]);
						if (bn.node_index() == bn1.node_index())
						{
							bMatch = true;
							if (bn.produced() > bn1.produced())
							{
								indexes[i] = bn1.block_number();
								bNeedUpdate = true;
							}
							break;
						}
					...
					if (bMatch == false)
					{
						indexes[i] = BlockNumberType(0, bn.node_index()).block_number();
						bNeedUpdate = true;
					}
				}
				...
				if (bNeedUpdate)
				{
					self->set_last_scanned_block_numbers(indexes);
					self->set_last_scanned_block_numbers_for_alp(indexes);
				}*/
                
                //if (_blockchain->get_head_block_num() < last_unlocked_scanned_number) {
                //    self->set_last_scanned_block_numbers(_blockchain->get_head_block_num());
                //    self->set_last_scanned_block_numbers_for_alp(_blockchain->get_head_block_num());
                //}
            }
            
            void WalletImpl::block_applied(const BlockSummary& summary) {
				if (!self->is_open()) return; // || !self->is_unlocked()) return;
				...
				//auto lastScaned = self->get_last_scanned_block_numbers();
				////for (int i = 0; i < lastScaned.size(); i++)
				////{
				////	BlockNumberType bn = BlockNumberType(summary.block_data.block_num);
				////	BlockNumberType bn1 = BlockNumberType(lastScaned[i]);
				////	if (bn.node_index() == bn1.node_index() && bn.produced() <= bn1.produced())
				////		return;
				////}         

				//
    //            //self->start_scan(std::min(self->get_last_scanned_block_number() + 1, summary.block_data.block_num), -1);
				////lastScaned = self->get_last_scanned_block_numbers();
				//bool find = false;
				//for (int i = 0; i < lastScaned.size(); i++)
				//{
				//	if (BlockNumberType(lastScaned[i]).node_index() == BlockNumberType(summary.block_data.block_num).node_index())
				//	{
				//		lastScaned[i] = std::min(lastScaned[i], summary.block_data.block_num-1);
				//		find = true;
				//		break;
				//	}
				//	//else 
				//	//	lastScaned[i] ++;
				//}
				//if (!find) lastScaned.push_back(summary.block_data.block_num-1);
				//self->start_scan(lastScaned, -1);
            }
            ...
            */
            ...
            SecretHashType WalletImpl::get_secret(uint64_t block_num,
            ...
            void WalletImpl::start_scan_task(vector<uint64_t> start_block_nums, const uint64_t limit) {
				try {
					fc::oexception scan_exception;
					try {
						vector<uint64_t> head_block_nums = _blockchain->get_index_list();
						vector<uint64_t> current_block_nums;
						int64_t total = 0;

						for (int i = 0; i < head_block_nums.size(); i++)
						{
							BlockNumberType bn0 = BlockNumberType(head_block_nums[i]);
							BlockNumberType bn1 = BlockNumberType(0, bn0.node_index());
							for (int j = 0; j < start_block_nums.size(); j++)
							{
								BlockNumberType bn2 = BlockNumberType(start_block_nums[j]);
								if (bn0.node_index() == bn2.node_index()) {
									bn1 = bn2;
									break;
								}
							}
							int64_t block_count = bn0.produced() - bn1.produced();
							if (block_count > 0) {
								total += block_count;
								current_block_nums.push_back(bn1.block_number());
							}
							else {
								bn1 = bn0;
								current_block_nums.push_back(head_block_nums[i]);
							}
						}
						...
						vector<SignedBlockHeader> headers;
						int header_count = 0;
						for (int i = 0; i < current_block_nums.size(); i++)
						{
							if (current_block_nums[i] < head_block_nums[i]) {
								headers.push_back(_blockchain->get_block_header(current_block_nums[i] + 1));
								header_count++;
							}
							else {
								headers.push_back(SignedBlockHeader());
							}
						}

						char buf[256] = "";
						string last_scanned_str = "";
						for (int i = 0; i < current_block_nums.size(); i++)
						{
							BlockNumberType bn1 = BlockNumberType(current_block_nums[i]);
							if (buf[0] != 0) last_scanned_str += ", ";
							sprintf(buf, "%d:%u", bn1.node_index(), bn1.produced());
							last_scanned_str += string(buf);
						}
						//printf("Scan started: %s (%d)\n", last_scanned_str.c_str(), total);

						//ulog("Scan started: ${n} (${m})\n", ("n", last_scanned_str)("m", total));
						while(header_count > 0) {
							int index = -1;
							for (int i = 0; i < current_block_nums.size(); i++) {
								if (headers[i].block_num > 0) {
									if (index == -1) index = i;
									else if (headers[index].rank > headers[i].rank) index = i;
								}
							}
							if (index == -1) break;

							try {
								scan_block(headers[index]);
							}
							catch (const fc::exception& e) {
								elog("Error scanning block ${n}: ${e}", ("n", current_block_nums[index])("e", e.to_detail_string()));
							}

							++count;
							current_block_nums[index] = headers[index].block_num;
							//BlockNumberType bn1 = BlockNumberType(current_block_nums[index]);
							//printf("Scan block: %d:%u\n", bn1.node_index(), bn1.produced());

							self->set_last_scanned_block_numbers(current_block_nums);
							update_progress(count);
							if (count % 10 == 0) fc::usleep(fc::microseconds(1));

							if (current_block_nums[index] < head_block_nums[index]) {
								headers[index] = _blockchain->get_block_header(current_block_nums[index] + 1);
							}
							else {
								headers[index] = SignedBlockHeader();
								header_count--;
							}
						}
				...
				FC_CAPTURE_AND_RETHROW((start_block_nums)(limit))
            }
                ...
                try {
                    ...
                    if (current_version < 111) {
                        ...
                        const function<void(void)> repair = [&]() {
                            ...
                            self->start_scan(vector<uint64_t>(), -1);
                        };
                        _unlocked_upgrade_tasks.push_back(repair);
                    }
                    
                    if (current_version < 112) {
						vector<uint64_t> newIndexes;
                        //self->set_last_scanned_block_number_for_alp(0);
						self->set_last_scanned_block_numbers_for_alp(newIndexes);
                    }
            ...
            void WalletImpl::create_file(const path& wallet_file_path,
                ...
                try {
                    ...
                    try {
                        ...
                        self->set_last_scanned_block_numbers(_blockchain->get_index_list());
            ...
            void WalletImpl::open_file(const path& wallet_file_path) {
                try {
                    ...
                    try {
						...
						wlog("Wallet opened : ${e}", ("e", _wallet_db.get_id()));
        ...
        void Wallet::open(const string& wallet_name) {
            try {
				...
				//auto lastScaned = get_last_scanned_block_numbers();
				set_transaction_scanning(true);
				//start_scan(lastScaned, -1);
            }
		...
		fc::sha512 temp_password;

		void Wallet::unlock_waiting(const string& password, uint32_t timeout_seconds) {
			try {
				std::exception_ptr unlock_error;
				...
				try {
					...
					temp_password = fc::sha512::hash(password.c_str(), password.size());

					if (!my->_wallet_db.validate_password(temp_password))
						FC_THROW_EXCEPTION(invalid_password, "Invalid password!");
		...
		void Wallet::unlock_later() {
			try {
				...
				try {
					...
					my->_wallet_password = temp_password;
					my->upgrade_version_unlocked();
					my->reschedule_relocker();
					wallet_lock_state_changed(false);
					ilog("Wallet unlocked until time: ${t}", ("t", fc::time_point_sec(*my->_scheduled_lock_time)));
        ...
        PublicKeyType Wallet::create_account(const string& account_name,
            ...
            try {
                ...
                if (num_accounts_before == 0) {
					...
					set_last_scanned_block_numbers(my->_blockchain->get_index_list());
					vector<uint64_t> indexes;
					set_last_scanned_block_numbers_for_alp(indexes);
                }
        ...
        void Wallet::start_scan(vector<uint64_t> start_block_nums, const uint64_t limit) {
            try {
                if (NOT is_open()) FC_CAPTURE_AND_THROW(wallet_closed);                
                if (NOT is_unlocked()) FC_CAPTURE_AND_THROW(wallet_locked);
                ...
                const auto scan_chain_task = [=]() {
                    my->start_scan_task(start_block_nums, limit);
                };
		...
		void Wallet::sign_block1(SignedBlockHeader& header, PublicKeyType signee)const {
			try {
				if (NOT is_open()) FC_CAPTURE_AND_THROW(wallet_closed);
				...
				//const AccountEntry delegate_entry = my->_blockchain->get_slot_signee1(header.timestamp, active_delegate_ids);
				//FC_ASSERT(delegate_entry.is_delegate());
				const PublicKeyType public_signing_key = signee; // delegate_entry.signing_key();
				...
				//const uint64_t last_produced_block_num = delegate_entry.delegate_info->last_block_num_produced;
				...
				//	const map<uint64_t, PublicKeyType>& signing_key_history = delegate_entry.delegate_info->signing_key_history;
				//	const uint64_t last_signing_key_change_block_num = signing_key_history.crbegin()->first;
        ...
        void Wallet::sign_block(SignedBlockHeader& header)const {
            try {
				...
				const uint64_t block_num = header.block_num;
                const vector<AccountIdType>& active_delegate_ids = my->_blockchain->get_active_delegates();
				const AccountEntry delegate_entry = my->_blockchain->get_slot_signee(block_num, active_delegate_ids);
                ...
                //header.next_secret_hash = fc::ripemd160::hash(my->get_secret(header.block_num, private_signing_key));
				//header.delegate_sign = my->get_secret(block_num, private_signing_key); 
				header.sign(private_signing_key);
                FC_ASSERT(header.validate_signee(public_signing_key));
            }
        ...
        WalletTransactionEntry Wallet::register_contract(const string& owner, const fc::path codefile, const string& asset_symbol, double init_limit, bool is_testing) {
			...
			//fee.amount = 0; // LUCKY 2019/6/1 UPDATED
        ...
        WalletTransactionEntry Wallet::transfer_asset_to_address(
            ...
            try {
				...
				//rao modifications
				ShareType minfee = 10000; //0.1 TTC
				double transfee = 1000 * get_transaction_fee_ratio() * amount_to_transfer / 1000;
				
				if (asset_id == 0)
				{
					transfee = amount_to_transfer * 0.005; //0.5 percent of actual amount of TTC
        ...
        WalletTransactionEntry Wallet::issue_asset(
            ...
            try {
                ...
                trx.expiration = blockchain::now() + get_transaction_expiration();
                auto required_fees = get_transaction_fee();
                oAssetEntry asset_entry = my->_blockchain->get_asset_entry(symbol);
				AssetEntry assetEntry = *asset_entry;
        ...
        void Wallet::set_last_scanned_block_numbers(vector<uint64_t> block_nums) {
            try {
                FC_ASSERT(is_open(), "Wallet not open!");
                my->_wallet_db.set_property(last_unlocked_scanned_block_number, fc::variant(block_nums));
            }
            
            FC_CAPTURE_AND_RETHROW()
        }
        void Wallet::set_last_scanned_block_numbers_for_alp(vector<uint64_t> block_nums) {
            try {
                FC_ASSERT(is_open(), "Wallet not open!");
                my->_wallet_db.set_property(last_scanned_block_number_for_thinkyoung, fc::variant(block_nums));
            }
            
            FC_CAPTURE_AND_RETHROW()
        }
        vector<uint64_t> Wallet::get_last_scanned_block_numbers()const {
            try {
                FC_ASSERT(is_open(), "Wallet not open!");
                
                try {
                    return my->_wallet_db.get_property(last_unlocked_scanned_block_number).as<vector<uint64_t>>();
                    
                } catch (...) {
                }
                
				return vector<uint64_t>(); // my->_blockchain->get_index_list(); // get_head_block_num();
            }
            
            FC_CAPTURE_AND_RETHROW()
        }
        vector<uint64_t> Wallet::get_last_scanned_block_numbers_for_alp()const {
            try {
                FC_ASSERT(is_open(), "Wallet not open!");
                
                try {
                    return my->_wallet_db.get_property(last_scanned_block_number_for_thinkyoung).as<vector<uint64_t>>();
                    
                } catch (...) {
                }
                
				return my->_blockchain->get_index_list(); // get_head_block_num();
            }
        ...
        variant Wallet::get_info()const {
            ...
            if (is_open) {
				...
				info["last_scanned_block_num"] = last_scanned_str;
