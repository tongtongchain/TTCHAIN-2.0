...
void WalletImpl::scan_block(SignedBlockHeader block_header) //uint64_t block_num)
{
    try {
        ...
        fc::async([=]
        {
            ...
            for (int i = 0; i < transaction_entrys.size(); i++)
            {
                ...
                scan_key_progress.push_back(_scanner_threads[i%_num_scanner_threads]->async([=]
				{
					try
					{
						scan_transaction(eval_state.trx, block_header.block_num, block_header.timestamp,true);
					}
					catch (...)
					{
					}
    ...
    } FC_CAPTURE_AND_RETHROW((block_header.block_num))
}
...
// 트랜잭션을 캐싱하는 함수
void Wallet::cache_transaction(WalletTransactionEntry& transaction_entry, bool seed, bool store)
{
	try {
...
bool Wallet::is_seed_node()
{
	if (!is_open() || is_locked())
		return false;

	vector<WalletAccountEntry> enabled_delegates = get_my_delegates(enabled_delegate_status);
...
void Wallet::scan_balances_experimental()
{
	try {
		my->scan_balances_experimental();
		//if (store)
    ...
    TrxType trx_search_type)const
{
    try {
        ...
        for (const auto& item : transactions)
        {
			...
			//history_entrys.insert(history_entrys.begin(), tx_entry);
    ...
    TrxType trx_search_type)const
{
    try {
        ...
        const auto& transactions = my->_wallet_db.get_transactions();
        AssetIdType asset_id = 0;
        if (!asset_symbol.empty() && asset_symbol != ALP_BLOCKCHAIN_SYMBOL)//²éµ¥Ò»ÀàÐÍ
        {
            try
            {
                asset_id = my->_blockchain->get_asset_id(asset_symbol);
        ...
        for (const auto& item : transactions)
        {
			const auto& tempTx_id = TransactionIdType(item.first);
			const auto& tempTx_entry = TransactionData(item.second);//¹ýÂËÓë²éÑ¯ ÀàÐÍ²»Æ¥ÅäµÄ×Ê²ú
            ...
            if (asset_id != 0/*!asset_symbol.empty()*/)//²éÑ¯×Ê²úÊÇ·ñÆ¥Åä
            {
                bool match = false;
                for (const auto& entry : tx_entry.ledger_entries)
                    match |= entry.amount.amount > 0 && entry.amount.asset_id == asset_id;
            ...
            history_entrys.push_back(tx_entry);
			string str = tempTx_id.str();
			//ilog("TRX_ID {trx_id}", ("trx_id")(tempTx_id));
        }
        return history_entrys;
    } FC_CAPTURE_AND_RETHROW()
}
    ...
    bool wither_splite)const
{
    try {
		...
		for (const auto& item : history){//Ìí¼Ó
			...
			if (pretty_trx.trx_type == thinkyoung::blockchain::TransactionType::create_asset && trx_splite == trx_type_desipate){
				continue;
			}//Èç¹ûÊÇ´´½¨×Ê²ú²¢ÇÒÊÇ³äÖµoption£¬Ìø¹ý
        ...
        for (const auto& name : account_names)
        {
			...
			vector<PrettyTransaction>::iterator trx = pretties.begin();//Ìí¼Ó
                    ...
                    if (from_me)
                    {
                       ...
                       /* if ((running_balances[amount_asset_id] - entry.amount) >= Asset(0, amount_asset_id))
						...
						else if (trx.trx_type == thinkyoung::blockchain::TransactionType::transfer_multi_asset)
						{
							...
							if (asset_symbol == ALP_BLOCKCHAIN_SYMBOL)
							{
								//multi-asset transfer OP£ºspecify ALP_BLOCKCHAIN_SYMBOL
								if (!is_abolished)
									running_balances[amount_asset_id] += entry->amount;
						...
						{
							//´úÀíÁì¹¤×Ê£¬²»»á¸Ä±ä¸Ã´úÀíÕË»§µÄ¿ÉÓÃÓà¶î£¬ÕâÀï²»ÓÃ¿Û³ý
							if (!is_abolished)
								running_balances[amount_asset_id] += entry->amount;
						}
						////////////////////////////////////////////////////////////////////////////
...
PrettyTransaction		Wallet::to_pretty_trx(const thinkyoung::blockchain::TransactionEntry& trx_entry, const std::string addr_for_fee) const
{
    ...
    for (const auto& op : trx.operations)
    {
		...
		case update_account_op_type:
		{
            ...
            //ËùÒÔÏÖÔÚÖ»ÄÜ¼òµ¥´Ö±©µÃÓÃÒ»¸ö1000ALPÀ´½øÐÐ»®·Ö£¬Ö»ÒªÊÇ´óÓÚ1000ALP¶¼ÈÏÎªÊÇÉý¼¶´úÀí
...
PrettyContractTransaction		Wallet::to_pretty_contract_trx(const thinkyoung::blockchain::TransactionEntry& trx_entry) const
{
    ...
    uint64_t block_num = trx_entry.chain_location.block_num;
